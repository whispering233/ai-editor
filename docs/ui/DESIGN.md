...
**缩进列对齐（2026-09 修正）**：缩进行的折叠箭头（`size="small"` icon-only，宽 24px）与**无子节点占位必须同几何**——占位写成 28px 会让「有子节点 / 无子节点」两类行的类型徽标、标题、摘要第二行、就地新建行各差 12px（大纲页曾是此状态）。

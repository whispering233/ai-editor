...
- **占用段三态**：① `percent` + `tokens` 有值 → 2px 圆角条 + `42% · 1M`；② 只有 `contextWindow`（压缩后 `tokens` 为 `null`）→ `? · 1M` 且**不画条**；③ 连窗口都没有（无模型 / 设置未加载）→ 整段隐藏。条填充色按占比经 antd token 取色（`CONTEXT_ERROR_PERCENT` 以上 `colorError`、`CONTEXT_WARN_PERCENT` 以上 `colorWarning`、其余 `colorPrimary`），**禁硬编码色值**。

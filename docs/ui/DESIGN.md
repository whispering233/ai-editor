...
**`book-delete-dialog`（删书确认框）** — 复用受控 Dialog：书名 + 三行后果（本地目录与 `.backups/` 一并删除、不可恢复；已启用云备份时会先把当前状态推送到云端；在跑的拆解任务会被取消）+ **默认不勾选的复选项「同时删除云端备份」**；Footer = `[取消]` + `[删除]`（**danger**）。云前置推送失败 → 框内错误文案 + Footer 换成 `[取消]` + `[仍要删除]`（danger，`force` 路径；文案写明「最新改动不会上传云端」）。删除成功后回书架，并刷新书架 / 项目配置 / 云端状态。

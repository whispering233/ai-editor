...
**`cloud-pull-confirm`（拉取确认框）** — 复用 `components/ui/dialog.tsx`（受控）：展示云端那份的元信息（时间 / 类型 / 标签 / 设备 / 统计 / 大小）+ 两句后果声明（「本机当前状态将先自动快照到本地备份」「正文/参考资料会随三文件被云端那份覆盖；本机对话历史不受影响——会话不进云端」）；Footer = `[取消]` + `[拉取]`（`button-primary`）。

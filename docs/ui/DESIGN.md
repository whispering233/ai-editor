...
- **「问 AI」悬浮球（中栏右下）的页面语义**：无行级焦点时按页面注入——**大纲组页面**（`#/outline` / `#/outline/:nodeId`）在项目**存在推演节点**时注入**推演节点集合**（`title` = 「带着推演节点去问 AI」，focus 小条同步显示「推演节点 · N 个」）；其余情况维持现状（无焦点 → 普通进入聊天 + 中性提示）。注入内容由服务端现读项目配置展开（`20-context.md` §2）。
...
**`focus-strip`** — 「正在讨论：{类型} {名称}」小条：`{colors.surface-soft}` 底 + 1px 描边 + caption。**推演形态（2026-09）**：注入的是标记**集合**而非单条对象（无名称可解析）⇒ 文案 = 「正在讨论：推演节点 · N 个」（N = 当前项目推演节点数）。

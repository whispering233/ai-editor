...
3. **同步状态**（**卡 4/5 落地**：卡 3 只有账号配置与自动推送两段；`GET /cloud/status` 当前仅返回配置段六字段）：状态行（云端最新份 = 时间 · 类型 · 标签 · 设备 · 统计 · 大小；本机已推份；上次同步时间；未推改动标记）+ **云端份列表**（时间倒序 ≤ 5 行，行语言同 `data-row`，行尾标「云端最新」）+ 动作 `button-default`「推送」/「拉取」。失败态：错误文案 + `button-default`「重试」；未配置：`empty-state` + 引导。

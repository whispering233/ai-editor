...
- **进度节点选择器只列章**（`chapterNodeOptions`，与 `HookPanel` 的章节点选择器同一 helper）：状态按**章序前缀**累积，非章节点只是某章的**别名**（场景 → 所属章、卷 → 该卷最后一个未软删章、root → 章序 0 = 初始值），列出来只会让下拉变长并造成"粒度更细"的错觉。**通用 compute 探针**（设定/地点/伏笔/时间点详情页的 `compute-preview`）同样只列章。契约层不变：`POST /delta/compute` 与工具 `compute_state` 的 `at_node_id` **仍不限层级**（非章入口只存 API/工具层）。
...
**`character-relations`（人物关系网 tab）** — tab 内容 = 一张 `card`：**一行操作区**（右对齐 `button-default`「+ 添加人物关系」）+ **星形图（`relation-star-graph`，仅达阈值时）** + 分组行列表；**不另画区标题/不放折叠壳**（tab 标签已是区名）。行语言 = `data-row`（底部 1px `{colors.hairline-soft}`、hover `{colors.surface-soft}`）：按 `relationType` **分组**（组头 = `section-title` 字号档中的 caption 行 + 条数，**条数 = 去重后行数**），行 = 对方姓名（可点击切选中该角色）+ 方向箭头（`→` / `←`，双向边标「双向」徽标）+ `metadata` 备注副行（`caption-text`）+ `icon-button` 删除；添加入口的关系类型控件 = **`select-free-input`**（选项 = **调用方子集**（人↔人 5 类：`ally`/`rival`/`mentor`/`family`/`kills`）∪ 「本项目已用的**自定义**类型」（不在预定义表里的，带条数）——子集之外的预定义类型不因「被用过」而回填），**目标端类型锁定 `character`**（只读卡片，无下拉）。**对称关系显示去重**（`ally`/`rival`/`family` 同时存在两条边时合并一行 + 「双向」），但**不自动建反边**（显示层去重，不做双写）；**合并行删除只删方向边（out）那一条**，确认文案需声明只删其中一条。仅展示以本角色为一端的关系（其余 = 其他关联）。

**`relation-star-graph`（人物关系星形图，2026-09）** — 「人物关系网」tab 内、分组列表**之上**的纯展示图（同一数据集：图 = 索引、列表 = 明细；不加 tab、不加路由、不发新请求）。中心 = 当前角色（`{colors.primary}` 实心点 + 姓名，姓名在下方）；叶子 = **按对方人物去重**（同一人的多条关系合并为一叶，条数 > 1 时名字后缀 ` · N` —— 一个叶子对应列表多行时的诚实标注）；连线 1px `{colors.hairline-strong}`，有向画箭头（out 朝外 / in 朝内），**该人同时存在 out 与 in（或任一行是对称合并行）→ 不画箭头**；**自环不进图**。叶子名 12px 截断到 8 字、色取 `{colors.secondary}`（实现为 `colorText` 80% 近似值——Tailwind 未暴露该 utility，同 `thinking-block` 登记）；**叶子可点** = 切到该角色（与列表行同行为）；**不做**拖拽 / 缩放 / 多跳 / 按类型着色（类型信息由列表承担；token 唯一入口纪律）。半径随叶子数自适应，**画布高度随半径长高**（下限 220px；纵向预算 480px，当前宽度预算下实际最多约 400px——否则约 10 叶起半径饱和、叶子挤成一圈）；**标签横向余量不足时锚点翻向内**（叶子多 + 长名字时不被视口裁字尾；半径公式只管纵向，横向余量 = 宽度 − 极值叶位置）。**触发阈值**：去重后**叶子数** < 4 不渲染（零星关系零信息量）。**已知边界**：叶子 > 24 时只画点不画名字（拥挤无解）。**实现口径**：零依赖手写 SVG（1 跳星形 = 极坐标，引入布局库无收益；多跳 / 拖拽 / 缩放出现时再评估 `d3-force`）。

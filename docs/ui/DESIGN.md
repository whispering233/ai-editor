...
**`relation-star-graph`（人物关系星形图，2026-09）** — 「人物关系网」tab 内、分组列表**之上**的纯展示图（同一数据集：图 = 索引、列表 = 明细；不加 tab、不加路由、不发新请求）。中心 = 当前角色（`{colors.primary}` 实心点 + 姓名，姓名在下方），叶子 = 关系网去重后的对方节点，半径随叶子数自适应；连线 1px `{colors.hairline-strong}`，有向画箭头（out 朝外 / in 朝内），对称合并行与自环不画箭头（**自环不进图**）。叶子名 12px 截断到 8 字（`{colors.secondary}`），**叶子可点** = 切到该角色（与列表行同行为）；**不做**拖拽 / 缩放 / 多跳 / 按类型着色（类型信息由列表承担；token 唯一入口纪律）。**触发阈值**：去重后行数 < 4 不渲染（零星关系零信息量）。**已知边界**：叶子 > 24 时只画点不画名字（拥挤无解）。**实现口径**：零依赖手写 SVG（1 跳星形 = 极坐标，引入布局库无收益；多跳 / 拖拽 / 缩放出现时再评估 `d3-force`）。

...
3. **同步状态**（**卡 4 落地**：云端最新份 / 本机最新份 / 「推送到云端」按钮 + 冲突时的行内「用本机覆盖云端」入口；**拉取与三态状态机属卡 5/6**，届时冲突分支改为 `cloud-conflict-dialog`）：状态行（**卡 4 已落**：云端最新份 = 时间 · 类型 · 标签 · 设备 · 统计 · 大小；本机最新份；云端检查失败的 `errorCode` 行）+ **本机已推份 / 上次同步时间 / 未推改动标记（卡 5）** + **云端份列表（时间倒序 ≤ 5 行，行语言同 `data-row`，行尾标「云端最新」）（卡 5）** + 动作 `button-default`「推送」（卡 4）/「拉取」（卡 5）。失败态：错误文案 + `button-default`「重试」；未配置：`empty-state` + 引导。

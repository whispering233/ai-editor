...
- **`gh release upload` 按「本地文件名」命名资产（2026-09-16 实测踩三次）**：想覆盖 Release 上的 `latest.yml`，必须让**本地文件就叫 `latest.yml`**（放到专用目录里再传）——传 `backup/latest.yml.bak` 或 `/tmp/orig-latest.yml` 只会**新建**一个同名资产而**原 `latest.yml` 不会被覆盖**（`--clobber` 只覆盖同名）。验收方式：传完必须重新 `gh release download` 一次并比对 `sha256` 与资产清单（多出杂项资产就是没传对）。

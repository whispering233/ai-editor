...
- **会话归属**：会话文件在项目目录 `sessions/` 内，归属由目录表达，不依赖 data.db（改名/移动目录天然携带；但它是**纯本地目录**——不进备份/导出/云端，不跨机器搬运）。拆解会话（`decompose-` 前缀，每 job 一枚）与 chat 会话同目录但**只读**：`POST /chat` 拒它、有在途 job 时禁删（见 `60-decompose.md` §2.1）。

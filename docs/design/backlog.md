...
- **批大小上调 / 自适应（待实测依据）**（2026-09 讨论登记） — 现状：`DECOMPOSE_BATCH_TARGET_CHARS` 是历史默认值、无实测依据；方案 = 样本书四档对照（当前值 / 2× / 4× / 全文一批），结论回写设计文档与常量注释。触发条件：实测显示质量不退且吞吐更优 → 上调并给出上限；反之写明理由、不再重问。

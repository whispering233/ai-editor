...
- **编号重启自动分卷** — 现状：只提示「疑似多卷」，结构上仍单卷「全书」（`root` 仅接纳卷，单卷是必顶兜底）。触发条件：多部/副本类网文（编号多次重启）的单卷长列表真正难用。升级路径：以编号重启点作卷边界，仅在重启次数落在合理区间时采用（实测一份样本 765 章里 36 处重启）。
...
- **并发调度** — 现状：串行（`DECOMPOSE_CONCURRENCY` 为常量，一处可调）；数百章的书要跑小时级。触发条件：真实出现「太慢」反馈。升级路径：并发 2–3 + 自建退避/限流兜底（**不在 pi 的重试链里**，429/超时要自己兜）。
- **性能基准** — 现状：无基准，只有人工真机记录。触发条件：真要调批大小时。最小修法：记录样本书（数十章 / 数百章各一）的批数、耗时、实际 token。
...
- **拆解耗时（串行）**（2026-09 真实跑登记） — 现状：48 章 / 253k 字样本 = 25 批串行 ≈ **33 分钟**（~80s/批，真实模型延迟 + medium 思考档），加 S3/S4 约 39 分钟；进度页可见但无并发。触发条件：用户抱怨慢。升级路径：调 `DECOMPOSE_CONCURRENCY` > 1，需自己兜 429/限流（不在 pi 的重试链里）。

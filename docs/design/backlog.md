...
- **`sessions/` 仍在备份与云范围内** — 现状：拆解会话里含整本原文副本，随备份 zip 上云（每轮一份，见 `60-decompose.md` §7.2）。触发条件：用户决定把会话排除出备份/云。最小修法：改 `PACKED_DIR_NAMES` 口径——**四处必须同步**（备份白名单 / 打包 / 变更判定 / 恢复），并且要定「现有备份与云端的 `sessions/` 条目如何处置」（兼容读取还是丢弃）。

...
- **拆解耗时（串行，无基准）**（2026-09 真实跑登记） — 现状：`DECOMPOSE_CONCURRENCY` 为常量、恒为 1（刻意：不在 pi 的重试链里，429/限流要自己兜）；实测 48 章 / 253k 字 = 25 批 ≈ **33 分钟**（~80s/批：真实模型延迟 + medium 思考档 + 每批 5–9KB JSON 输出），加 S3/S4 约 39 分钟；数百章的书要跑小时级，且**无性能基准**（只有人工真机记录）。触发条件：用户抱怨慢、或要调批大小时。升级路径：先记基准（样本书的批数 / 耗时 / 实际 token），再调 `DECOMPOSE_CONCURRENCY` > 1 + 自建退避/限流兜底。

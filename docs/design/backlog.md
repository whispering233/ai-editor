...
- **正文只读模式** — 现状：**专注模式（2026-10）已落地**（只收三栏外壳 / `info-bar` / 页头），但**编辑器仍是可编辑态**（无只读切换）；阅读与打印体验仍缺。触发条件：需要纯净阅读/打印（或导出打印稿）。最小修法：同一个编辑器 `editable={false}` + 隐工具条的浮动条（入口已收在常显工具条右侧，只读 = 那里多一个开关；**注意** `editable` 变更会让 BlockNote 重挂 contenteditable，切换时不得丢失未保存输入）。
- **「页头常驻」尚未全站统一（2026-10 登记）** — 现状：参考资料列表/详情、人物工作台、时间轴、章正文页为**常驻**；概览 / 大纲 / 实体列表 / 设置 / 回收站仍走**外壳滚动**（页头随内容离开）。口径与实现方式已写进 `DESIGN.md` §中栏页头结构「页头常驻」。触发条件：逐页改造时（视觉同步做，不要一次全仓翻）或用户抱怨某页找不到操作按钮。最小修法：该页 `section` 改 `flex h-full min-h-0 flex-col` + 内容进内层 `flex min-h-0 flex-1 flex-col overflow-y-auto`。
- **写作面的剩余旋钮（行宽 / 选中字数 / 阅读模式）** — 现状：2026-10 写作面批次只做「字体 / 字号 / 行高 / 纸张 / 纹理 / 段首缩进 + 专注模式」；**行宽档位**（窄/默认/宽/全宽）与**选中字数**当时评估后被削；**只读模式**见上一条。触发条件：写作时长上来后出现「行太宽难扫行」或「想知道这段多少字」的真实抱怨。最小修法：行宽 = `[data-writing-width]` 给 `.bn-editor` 上 `max-width` + `margin-inline: auto`（一行 CSS）；选中字数 = 监听 selection 变化（`onSelectionChange`）算选中文本长度。

...
- **卸载时删 `%APPDATA%\AI Editor` 可能「部分成功」**（2026-09-16 真机间接观测）— 现状：`customUnInstall` 用 `RMDir /r "$APPDATA\AI Editor"`，而那时桌面版的日志文件（`logs\ai-editor.log`）可能仍被即将退出的应用占用 ⇒ NSIS 可能删掉 `desktop.json` 却留下 `logs\`，于是下次启动日志出现「首次启动：使用默认书库位置」（`readDesktopConfig` 读不到就返回 null，会重写默认路径——**自愈，无功能损害**），但「清除使用数据」并不彻底。触发条件：用户选「是」且当时应用刚退出。最小修法：删 `logs` 前先 `Delete "$APPDATA\AI Editor\logs\ai-editor.log"`（或提示「若删除不净请重启后再试一次」）。

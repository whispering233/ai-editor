...
- **真云盘（坚果云）人工验收清单**（自动化环境只到「本地 WebDAV + mock fetch」；真实云盘的认证/配额/时区三项无法在 CI 覆盖）
  ...
  3. 另一台机器（或换一个创作根模拟）：同样的 WebDAV 配置 + 同一本书 → 打开项目 → 点左栏「同步云端」：若云端有份而本机无记录 → 期望弹**裁决框**；选「保留云端」后本机三文件被云端那份覆盖（覆盖前状态进 `.backups/`），**本机 `sessions/` 不动**（会话不参与同步）。
...
- **云端书架（全量同步）**（列云端全部书的目录、一键拉取到本机）
  - 现状（2026-09 起）：已做**「列 + 导入新书」**（`GET /cloud/remote-books` / `POST /cloud/import-book`，书架「从云端恢复…」）；未做逐书**冲突裁决**与一键拉全部、也无人端删除传播。
  - 触发条件：书多到「逐本点导入」开始痛，或需要以云端为书架视图。
  - 升级路径：在现有列表端点上加「与本机 id 比对」的分流 UI（已有 `localExists`）+ 一键逐本 import；冲突裁决复用 `cloud-conflict-dialog`。
- **一键推送整架书**（把未打开过的书也推上云）
  - 现状（有意）：推送挂在「当前打开的项目」上（自动定时 / 关闭项目 / 手动）；从未打开的书自然没上云，需逐本打开推一次。
  - 触发条件：「配好云盘后发现还有几本没备份」成为真实痛点。
  - 升级路径：遍历 `books/`，对未打开的书临时开 `data.db` 打包推送（注意：要串行、要跳过大目录、要复用 `pushBackup` 的冲突判定）。
- **文件级增量上传**（避免每次全量 `PUT` 整个 zip）
  - 现状（有意）：整包上传（云端文件 = 本地备份文件逐字节拷贝）——实现简单、无格式转换。**包体积 = 三文件**（2026-09 起 `sessions/` 不再进包），主要来自 `data.db` 内的正文本体。
  - 触发条件：单次推送体积/耗时可感知地变差（或云盘配额被反复烧穿）。
  - 升级路径：云端布局改为「一文件一对象」（`data.db` / `outline.json` 分别上传，只传变化项），拉取时本地重新打包成 zip 走现有校验管道；代价是冲突检测与保留策略的粒度全部重做。
- **内建端到端加密**
...
- **同步基准的毫秒截断 vs 文件系统 mtime 亚毫秒抖动**（2026-09 发布前审计登记）
  - 现状：`lastSyncAt` 是 ISO（毫秒）基准，`hasChangesSince` 对 `project.json` / `outline.json`（以及三文件口径下的其它文件）**严格 `>`** 比较；文件 mtime 带亚毫秒且与 `Date.now()` 的截断有抖动 ⇒ 同步完成同一毫秒内落盘的文件可能被读成「本机有改动」——表现为多一次「有未备份改动」/ 自动推送（无数据风险，仅提示噪声）。
...
- **存量拆解书缺 `project.json` 的 `origin`**（2026-09 登记）
  - 现状：`origin` 是新增可选字段；存量项目缺失时打开一次就着库内有无拆解 job 补标一次（幂等），**从未打开过的老拆解书会落在书架的「小说项目」组**。
  - 触发条件：用户报告「老拆解书归错组」且不愿打开一次。
  - 最小修法：书架列表时批量补标（需读每本 `data.db`，代价高，不推荐）；或接受现状（打开一次即归位）。
- **存量备份包仍含 `sessions/**` 条目**（2026-09 登记）
  - 现状：本地 `.backups/` 与云端旧 zip 里的会话条目读取时接受并忽略（不报错、不搬入）；不做重写迁移。
  - 触发条件：无（靠 20 份本地保留策略自然老化；云端 5 份同理）。
  - 最小修法：不修；若真要省盘，用户手动删 `.backups/` 里的旧份。
...
- **历史端点新增了进程级 `ModelRuntime` 依赖**（端到端验收 oracle 登记，低危）
...
- **拆解 job 级成本 / 耗时汇总**
  ...
  - 触发条件：用户问「拆这本书花了我多少钱」。
  - 升级路径：`decompose_jobs` 已有 `session_id`；需一个 job 级聚合（扫该 job 的主 + worker 会话文件，口径复用 `sessionUsage`）——注意跨段并发写盘的时间窗。
- **历史会话的占用百分比**
  - 现状：打开历史会话时占用段显示 `? · 窗口`（只知窗口、不知占用）——重建需要 pi 未导出的 `estimateContextTokens`（`getContextUsage()` 的内部口径），自建等于复刻一套估算。
  - 触发条件：上游导出该函数，或用户反馈「打开旧会话看不到占用」影响判断。
- **状态栏实时速度估算与首字延迟（TTFT）**
  - 现状：速度只在每条 assistant 消息生成完后报一次（解码速度），生成中无数字。实时数字需对增量做 chars→token 估算（本仓另一处估算口径在拆解预算里），会把同一个字段变成两套口径。
  - 触发条件：用户要求「生成中就能看到快不快」。
  - 升级路径：待估算口径先收口（与 `decompose/budget.ts` 同源）再加，且必须与实测值在同字段上区分标注。TTFT 可与速度同批做（`message_start` → 首个增量的时长，meter 已经在记这两个时刻）。
- **会话平均速度 / 活跃时长**
  - 现状：只报最近一轮；平均值需 meter 累加并通过 `usage` 下发（历史会话无数据，只能覆盖「本次打开期间」）。
- **缓存浪费（重计费）诊断**
  - 现状：状态栏只报缓存读/写量与命中率；pi 的 `/session` 另有 `Cache Re-billed`（本该命中却重付的部分，含 `idleMs` 与模型切换标记，`core/cache-stats.ts`）。
  - 触发条件：用户怀疑「缓存只在某些轮生效」。
- **状态栏点击展开统计面板**
  - 现状：明细走 hover 的原生 `title`（无消息计数 / 工具调用数 / 会话文件 / git 分支 / 活跃时长）。
  - 触发条件：用户要求「像 pi-web 那样点开看全量统计」。
- **会话列表里的每会话成本列**
  - 现状：`GET /chat/sessions` 列表不带 usage；「哪本书/哪个会话烧钱多」需要逐条打开。
  - 触发条件：用户提出成本审计需求（列表页聚合会多扫一遍全部会话文件，需先评估大书库下的耗时）。
...
- **正文只读模式** — 现状：**专注模式（2026-09）已落地**（只收三栏外壳 / `info-bar` / 页头），但**编辑器仍是可编辑态**（无只读切换）；阅读与打印体验仍缺。触发条件：需要纯净阅读/打印（或导出打印稿）。最小修法：同一个编辑器 `editable={false}` + 隐工具条的浮动条（入口已收在常显工具条右侧，只读 = 那里多一个开关；**注意** `editable` 变更会让 BlockNote 重挂 contenteditable，切换时不得丢失未保存输入）。
- **「页头常驻」尚未全站统一（2026-09 登记）** — 现状：参考资料列表/详情、人物工作台、时间轴、章正文页为**常驻**；概览 / 大纲 / 实体列表 / 设置 / 回收站仍走**外壳滚动**（页头随内容离开）。口径与实现方式已写进 `DESIGN.md` §中栏页头结构「页头常驻」。触发条件：逐页改造时（视觉同步做，不要一次全仓翻）或用户抱怨某页找不到操作按钮。最小修法：该页 `section` 改 `flex h-full min-h-0 flex-col` + 内容进内层 `flex min-h-0 flex-1 flex-col overflow-y-auto`。
- **写作面的剩余旋钮（行宽 / 选中字数 / 阅读模式）** — 现状：2026-09 写作面批次只做「字体 / 字号 / 行高 / 纸张 / 纹理 / 段首缩进 + 专注模式」；**行宽档位**（窄/默认/宽/全宽）与**选中字数**当时评估后被削；**只读模式**见上一条。触发条件：写作时长上来后出现「行太宽难扫行」或「想知道这段多少字」的真实抱怨。最小修法：行宽 = `[data-writing-width]` 给 `.bn-editor` 上 `max-width` + `margin-inline: auto`（一行 CSS）；选中字数 = 监听 selection 变化（`onSelectionChange`）算选中文本长度。
- **纸张纹理与文本行对齐** — 现状：横线/网格是固定间距的装饰纹理（默认关闭），**不随正文行盒高对齐**（`DESIGN.md` §写作面偏好 / Known Gaps 已登记）。触发条件：用户按「稿纸」预期用它写字并抱怨错位。升级路径（需先定口径）：把行高锁成整数倍（24px 的倍数）+ 段距归零，并对标题类块单独补相位——代价是牺牲自选行高与部分排版自由度，**不确认要写稿纸不启动**。
- **写作工具条的 a11y 名字（卡 13.5 登记，已试过一遍）** — 现状：工具条根元素只有 `role="toolbar"`、无 `aria-label`（库自带浮动条同样）。**不可用「多传一个属性」解决**：`Components.FormattingToolbar.Root` 只收 `className/children/onMouseEnter/onMouseLeave/variant`，其余键会被 core 的 `assertEmpty` 抛错（fixer 用探针实证：typecheck 绿、运行时崩）。触发条件：真需要屏幕阅读器识别该区域时。升级路径：自绘 Root（拿走 ariakit `Toolbar` 的那层包装）或给库提 PR；**别再走传属性那条路**。
- **参考资料详情页没有专注模式（有意）** — 现状：专注模式只给章正文页（`Q7 决策`：参考资料页一半是元数据表单，「专注」语义不成立）。触发条件：用户明确要求写长笔记时也能纯净界面。最小修法：给 `ReferenceDetail` 也传 `focus`/`status` + 沿用同一 AppShell 分支（但得先定「表单去哪」）。
- **写作面文案的手抄同源（卡 13.5 登记）** — 现状：帮助弹窗里「撤销 / 重做 / 写作设置 / 六档位名」是手写字面量，与 `editor-toolbar.tsx` / `writing-settings.tsx` 同名同值（库字典里没有这些名字）；快捷键与工具条按钮名已经字典同源。触发条件：改名时发现两处不同步。最小修法：把这几个名字提到一处导出常量（与 `shared/src/constants/delta.ts` 同思路），帮助弹窗与按钮共引用。
- **页面关闭 / 退出桌面版时的「最后 1.5s」可能丢（2026-09 登记，已确认口径）** — 现状：正文只靠空闲自动保存 + **React 卸载**时 flush；全仓 `beforeunload` / `pagehide` / `visibilitychange` / `keepalive` / `sendBeacon` **零命中** ⇒ 关标签页或点桌面版窗口 X（走主进程 `closeServer()`，进程内服务随之退出）时，最后 1.5s 内的输入可能没落盘。已加**手动「保存」按钮**（工具条右端，批 14）作为用户侧保底，但**不是自动兜底**。触发条件：出现真实的「关窗口丢一段字」报告。升级路径（两者都要定口径）：① 渲染侧 `pagehide`（或 `beforeunload`）+ `keepalive: true` 的 fetch（请求体 ≤64KB 限制，超限需另想办法或提示）；② 桌面侧主进程 `before-quit` 拦一次 —— 等渲染进程 flush 完成（或直接等一下在途 `PUT`）再 `closeServer()`。
- **写作工具条窄幅折行的顺序摩擦（卡 13.1 实测登记，低危）** — 现状：中栏窄于按钮总宽时工具条折行（已不裁切），但 `flex-1` 撑开块的右端组可能被挤到第二行靠左。触发条件：真实用户在小屏/三栏并行时抱怨按钮位置跳。最小修法：给右端组 `margin-left: auto` 或把状态区移到独立行。
...
- **参考资料正文没有分页读取（12.9 对照登记）** — 现状：章正文有 `get_chapter_text(offset, max_chars)` 分页拉取（截断时告知续读起点）；而 AI 取参考资料只有 `get_entity('reference')` 一次性返回全文投影——超长笔记撞上单条工具结果上限时被外层截断，提示语说「请缩小查询范围或分页获取」，但参考资料没有分页入口，模型只能放弃全文。触发条件：真出现「长参考资料（世界观/background 笔记）AI 读不完整」的疼痛。最小修法：`get_entity` 的 reference 分支加可选 `offset`/`max_chars`（与 `get_chapter_text` 同款分页 + 截断提示），或新增只读工具 `get_reference_text`；**不得**顺带加写工具。
...
- **删当前书时「在飞拆解批」的收尾无验证（2026-09 删书端点 oracle 登记）**
  - 现状：删当前书的顺序 = `closeProject`（先关 db）→ `setCurrentProject(null)`（取消在跑 job）→ 删目录；若恰有批在飞，该批可能在句柄已关 / 目录已删后继续写，行为由 runner 内部游走（未验证是否残留 unhandled rejection）。
  - 触发条件：真出现删书时控制台报错或进程异常。
  - 最小修法：真实跑一次「批执行中删当前书」，观察日志；必要时把顺序改为「先 `cancelRunningDecomposeJobs()` + 等当前批落定，再关连接删目录」。

- **换模型后续拆 / 重跑不重规划**（卡 4 oracle 登记）
  - 现状：批规划（含双向预算守卫）只在**建 job 时**落一次（批行 + `decompose_jobs` 快照）；`resume` / 单批 `rerun` 只按 job 快照重算段与滚动累积，**不重跑 `planBatchesWithinBudget`**。建 job 后换到更小窗口 / 更小 `maxTokens` 的模型再续跑时，旧批按**当前模型**在执行期预检里判定 ⇒ 超限批显式失败（文案明确，不落进 pi clamp），但作业不会按新模型重新拆批。
  - 影响：换小模型续跑会得到若干 `failed` 批（要逐批重跑或另开续拆 job 才绕过）；换大模型只是规划偏保守（无功能损失）。
  - 触发条件：用户真在 job 中途换模型并续跑（设置页改默认模型 / 换 provider）。
  - 最小修法：`resume` / `rerun` 前置加一次「按当前模型重算规划」——批数或切分与已落批行不同时重建**未完成批**的批行（`done` 批不动，段划分随之重算）；实现前先定「已落批行可否改写 seq」的口径（批行主键含 seq）。

- **编辑切分（重命名 / 合并 / 拆分章）** — 现状：预览**只读**（章列表 + 警告），用户无法修正切分错误；切错了只能重来或手动改大纲。触发条件：真实出现「这本书切错了且无法绕过」。升级路径：预览页改可编辑列表（重命名/合并/拆分）+ 同步调整正文块归属——是一个独立量级的功能（参考项目 `graph-every-novel` 单独做了一整块 UI），不要顺手塞进 MVP。
- **自定义切分规则** — 现状：候选模式表与阈值固定在代码（拆解模块）。触发条件：出现规则表覆盖不了的书（非常规装饰符 / 特殊卷命名）。升级路径：**字段形状对齐 Legado `TxtTocRule`**（`name` / `rule` / `isVolume` / `isExclude` / `enable` / `serialNumber`）——社区有大量现成正则，用户可直接粘贴；必须带正则 ReDoS 防护（参考 `zh-chapter-splitter` 的 `build_custom_pattern`）。
- **编号重启自动分卷** — 现状：只提示「疑似多卷」，结构上仍单卷「全书」（`root` 仅接纳卷，单卷是必顶兜底）。触发条件：多部/副本类网文（编号多次重启）的单卷长列表真正难用。升级路径：以编号重启点作卷边界，仅在重启次数落在合理区间时采用（实现口径实测：一份样本 757 章里 17 处重启）。
- **卷章同行只发卷切点（罕见形态）** — 现状：一行既含卷又含章标记时按卷处理，章切点被吞（后续文本归入前一章）、卷标题带上未剥离的章标记。触发条件：真实碰到这类 txt（罕见——多数 txt 的卷/章标题各占一行）。最小修法：同一行发两个切点（卷边界 + 章边界）并从章标题里剥掉卷前缀，需同步改评分/测试。
- **Delta / 伏笔 / 事件抽取（第二批维度）** — 现状：MVP 只拆六件套（章摘要 / 人物 / 设定 / 地点 / 关系 / 报告）；Delta 与伏笔已论证**逐章抽噪音大、需全书视野**。触发条件：六件套的抽取质量与归并口径得到验证后。升级路径：同一条管线加 schema 字段 + 归并规则（Delta 锚点限章、伏笔需 plants/advances/resolves 三类关系），增量成本低——**不要为此另起一条管线**。
- **拆解报告的标题块** — 现状：报告正文是纯文本段落（服务端按行拆块，不做 markdown 语义转换，与参考资料导入同口径），在编辑器里层级不清。触发条件：用户抱怨报告读起来没层次。升级路径：由管线直接产出 heading 块数组（需固定 BlockNote 块形状，与「服务端不解析块结构」的边界不冲突——是我们自己产出的块）。
- **书架「删除此书」入口** — 现状：无 `DELETE /project` 端点与 UI；拆解撤销靠删目录或恢复拆解前的自动备份 zip。触发条件：用户要求，或拆解结果不满意时的撤销路径不够直白。升级路径：加删书端点 + UI（含二次确认；考虑是否进回收站）。
- **拆解期间手工建同名实体的冲突** — 现状：不锁界面（拆解中可自由编辑），S3 归并靠 name 复用 + `merge_written` 的 `updated_at` 快照保护用户编辑。触发条件：真实撞上「拆解归并覆盖/重复了我手工建的人物」。最小修法：S3 归并前把「库内已存在但不在候选集合里的同名实体」列入报告提示。
- **卷首目录页启发式的误判风险** — 现状：连续多个超短段判为卷首目录页丢弃（预览会显示丢弃数）。触发条件：极短章的书被误判。最小修法：预览里给「不丢弃目录页」开关（阈值常量已单点）。
- **正文增量导入（书后又写了新章）** — 现状：续拆只能拆「已导入但未拆」的章（正文来自 S1 的一次性全量导入）；书后续新增的章节无法入项目。触发条件：用户写完新章想接着拆。升级路径：独立的「追加正文」入口（重传文件或粘贴新章）→ 切分 → 追加到 `outline.json` + `document_records`（需定口径：章序追加与已有差集的冲突、卷归属、重复导入去重）。
- **报告 / 全书摘要不注入拆解快照** — 现状：快照只给「已有名字 + 关系 + 前置章摘要」；拆解报告里的全书剧情摘要不进提示词（YAGNI）。触发条件：实测发现续拆时模型丢主线。升级路径：把报告前两段（作品概览 + 全书剧情摘要）作为起始快照的最高优先级块注入（需解析报告结构或另存结构化摘要）。
- **chat 面板不渲染 custom 过程条目** — 现状：拆解会话在 chat 面板只显示真实 user/assistant（完整回放），`custom` 过程条目需要扩 `projectMessageForWire` 才能显示（当前返回 null）。触发条件：用户抱怨聊天面板看不出「哪批是哪批」。最小修法：给 custom 条目加一条只读投影 + 客户端小卡渲染（与 `#/decompose` 时间线共用文案）。
- **拆解记录不能导出** — 现状：过程时间线与会话记录只能在应用内看；`sessions/` 里的 JSONL 对用户不可读。触发条件：用户要存档/分享拆解过程。升级路径：导出为纯文本/markdown（时间线级别，不倾倒 JSONL）。
- **跨轮「认领」后的重跑语义（2026-09 oracle 验收登记）** — 现状：跨轮行被本轮增量更新后会进本 job 的 `merge_written` 清单（换取「写入后刷新 `updated_at`」这条硬要求）；因此**该 job 之后再重跑**时，若新产物里没有它，会按本 job 语义软删（实体）/ 物理删（关系）。触发条件：用户对同一 job 反复重跑且同时动了其他批。可选修法：清单条目加来源标记（本轮写入 vs 跨轮认领），重跑只对前者生效（需改清单形状）。
- **会话清理的两个残余（2026-09 oracle 验收登记）** — ① 外层 catch 会把「已成功删除但后续 job 的会话查找抛错」吞成 0（删除已发生却不上报，极罕见）；② 保留窗口外且状态为 `paused` 的 job 会话会被删（契约只护「在跑」）⇒ 用户之后续拆那枚 job 时，过程记录从头开始（数据不受影响）。触发条件：真碰到「明明删了却查不到记录」或「续拆旧 job 看不到旧过程」。
...
- **测试文件里的包名 typo（旧账）** — 现状：`packages/client/src/lib/character-workbench.test.ts` 的 import 写成 `@whispering2333/ai-editor-shared`（多一个 `3`）；type-only + `.test.ts` 不进 `typecheck` ⇒ 静默，测试内类型错误永不被发现（同型 typo 已在切分卡内修过一处）。触发条件：下次触碰该文件时。最小修法：改包名（一行）。
- **归并的两个已知边界（merge oracle 登记，均在 21.7 接线前无用户可见影响）**
...
- **章被物理删除后的批素材剔除未跑用例**（21.6 oracle 登记） — 现状：`runner.ts` 有只读码但无用例；429/限流与并发路径已由分段并发实现承接（设计 §2.2）。触发条件：真实遇到拆解期间删章。
...
- **`Typography` 不在 antd 根元素守卫名单里（盲区）**（21.8 oracle 登记） — 现状：`design-discipline.test.ts` 的 `ANTD_GUARDED_COMPONENTS` 只扫 `Button` / `Input`，`Typography.Text` 上挂的 Tailwind 字号类会被 antd 无层 CSS 压掉（实测 `text-xs` 挂上、渲染仍是 14px）。触发条件：新增 antd 组件到守卫清单时（扩名单会连带扫描既有 13 处 `Typography` 用法，需一张独立卡）。最小修法：先把已发现的死类删掉（`decompose-dialog.tsx` 一处已随 21.9 收尾），再评估扩名单。
- **antd `Typography` 自带字体栈与 `--font-sans` 不一致**（21.8 oracle 登记，非本卡引入） — 现状：antd 字体栈是 Latin+Emoji 栈，Linux 上数字与空格会落到彩色 emoji 字体（实测警告行「39」字形异常），Windows/macOS 命中系统字体无影响；既有先例（`Trash.tsx` 计数、云备份面板时间）同款。触发条件：出现 Linux 用户抱怨字体怪异。最小修法：给 antd 字体栈补 `PingFang SC` / `Microsoft YaHei`（或给 `--font-sans` 对齐），需全站核一遗像素。
- **批结果数组内的 null / undefined 元素仍会抛**（21.9 oracle 登记） — 现状：`arrayOrEmpty` 只守「字段缺失/非数组」，`{chapters:[null]}` 这类数组内非对象元素仍抛 ⇒ 整页被 ErrorBoundary 换掉（浏览器已复现）；服务端 zod 管线产不出该形状（只能来自手工改库），可达性窄。触发条件：真出现手工改库/版本错配导致的整页崩。最小修法：`arrayOrEmpty` 内过滤非对象元素（≤ 2 行 + 1 条断言）。
- **重跑入口守卫仍是源码形态扫描**（21.9 oracle 登记） — 现状：正锚钉的是 `{canRerunBatch(current.status, batch.status) && (` 的精确拼写；写成外层包裹（`batch.status === "failed" && (<>{canRerunBatch(...) && <Button/>}</>)}`）时两条断言都放过；行为层已由像素证明正确（done 态 3 个按钮真实可点）。触发条件：真出现「重跑入口被收窄」的回归。升级路径：抽「哪些批显示重跑」纯函数 + 单测（页面因 store + SSR 拿不到 job，源码扫描是登记过的折衷）。
- **别名归并的候选上限会截断（实测有损）**（2026-09 真实跑登记） — 现状：`DECOMPOSE_ALIAS_CANDIDATE_MAX` 按提及次数截断候选，48 章样本上超限 ⇒ 13 个别名组因「名字不在候选集合」被保守丢弃（实体 379 时候选面明显不够）。触发条件：真实拆解后人物别名没合并上（同一人两条实体）。最小修法：提高上限，或按共现图分批归并（每批候选集内归并 + 跨批二次合并）。
- **设定/地点无跨章阈值（实测偏多）**（2026-09 真实跑登记） — 现状：只有人物与关系有 `DECOMPOSE_MENTION_MIN_CHAPTERS` 门槛，设定/地点靠提示词约束；48 章样本产出 **设定 231 / 地点 92 / 人物 56**，低价值条目占多数。触发条件：用户抱怨设定列表噪音。最小修法：设定/地点也加跨章阈值（或按提及次数取 top N）——注意这是**产品口径变更**，需先改设计文档。
- **拆解性能基准（2026-09 两档实跑已回填）**（2026-09 真实跑登记） — 现状（同一样本 = 48 章 / 253k 字 = 25 批）：
  - 串行（并发 1）基线：墙钟 ≈ **33 分钟**（≈ ~80s/批），加 S3/S4 ≈ 39 分钟；
  - 并发 4：墙钟 **981.4s**（抽取 866.5s + S3/S4 114.8s）、**零失败**、2 次缺章重试（批 10、11）；
    并发下每批均延迟 ≈ **105s**（较串行 ≈ 80s 膨胀 ≈ **32%**）。
  - 读法：并发净收益小于并发数（provider 侧并发下每批更慢）；实际 token 未按档逐批留档。
  - 触发条件：要调 `decompose.concurrency` 默认值 / `DECOMPOSE_BATCH_TARGET_CHARS` 档位时——本条目即对照基线（再跑一轮时顺手补 token 数据）。
- **批大小上调 / 自适应（待实测依据）**（2026-09 讨论登记） — 现状：`DECOMPOSE_BATCH_TARGET_CHARS` 是历史默认值、无实测依据；方案 = 样本书四档对照（当前值 / 2× / 4× / 全文一批），结论回写设计文档与常量注释。触发条件：实测显示质量不退且吞吐更优 → 上调并给出上限；反之写明理由、不再重问。**（实测已落地：四档对照完成、判定 = 保持，见 `60-decompose.md` §4「批大小口径」；剩样本外部分见下条。）**
- **大样本单批可行性未实测**（批大小实测的样本外残留） — 现状：全文一批档只在 48 章 / 253k 字样本上真跑过（1 次调用成功、覆盖完整）；文档里更大样本（757 章 / 约 212 万字 ≈ 1.4M 输入 token）属**样本外外推**——只论证了「任何现有窗口都装不下」，没有真跑过「装得下时会不会超窗 / 截断 / 失败」。触发条件：真要用全文一批拆大书（或出现超长窗口模型想复评该档）。最小修法：换一份真实大样本跑一次 `node scripts/decompose-batch-experiment.mjs run --tier full`（或更小的探针：只发一次全量输入，看是否超窗 / 截断 / 失败）。
- **段级进度可视化**（分段并发登记） — 现状：进度页按批展示、不区分段；N 段并行时看不出「哪批属于哪段 / 各段进度」。触发条件：用户觉得进度难读。最小修法：批列表加段号列或按段分组（段可由批序 + 快照并发数推导，无需新列）。
- **限流降档策略**（分段并发登记） — 现状：并发下的 429 / 限流只做指数退避 + 抖动，无「连续失败自动降档并发」。触发条件：真实 provider 在 N 路并发下频繁 429。最小修法：连续失败降档 + 冷却恢复（参数单点）。
- **并发数只能手编配置**（分段并发登记） — 现状：创作根 `.ai-editor/config.json` 的 `decompose.concurrency`，无 UI。触发条件：非技术用户需要调整时。最小修法：设置页加读写（存储仍回同一文件）。
- **报告第 6 段标题不精确**（2026-09 oracle 登记） — 现状：标题「被阈值滤掉的单章线索」现在还承载「悬空关系（端点幻觉）」类产物（`merge-write.ts` 注释已承认两类）。触发条件：用户对报告措辞提出疑问。最小修法：改标题/文案（如「被阈值或悬空过滤的线索」）。
- **decompose 模块一批 `export` 只服务同文件**（2026-09 收尾盘点登记，低） — 现状：约 20 个导出无跨文件消费方（`merge-write.ts` 的 `buildAliasCandidates` / `aggregateMentions` / `appendAliasNote` / `writeChapterSummaries` / `reportChapters` 等、`merge.ts` 的 `applyMentionThreshold` / `filterDanglingRelations`、`runner.ts` 的 `isRateLimitError` / `batchChapterHeading` / `batchUserMessage`、`report.ts` 的 `DECOMPOSE_REPORT_PLOT_MAX_CHARS`、db `queries/decompose.ts` 的 `DECOMPOSE_JOB_ID_PREFIX` 等；`split.ts` 的 `SPLIT_*` 阈值常量是文档按名引用的可调参数，另计）。触发条件：下次触碰这些模块时。最小修法：去掉多余 `export`（保持同文件使用）；将来真需要外部消费再补。

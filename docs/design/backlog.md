...
- **真云盘（坚果云）人工验收清单**（自动化环境只到「本地 WebDAV + mock fetch」；真实云盘的认证/配额/时区三项无法在 CI 覆盖）
  ...
  3. 另一台机器（或换一个创作根模拟）：同样的 WebDAV 配置 + 同一本书 → 打开项目 → 点左栏「同步云端」：若云端有份而本机无记录 → 期望弹**裁决框**；选「保留云端」后本机三文件与 `sessions/` 按并集合并（本机独有对话不丢），覆盖前状态进 `.backups/`。
...
- **文件级增量上传**（避免每次全量 `PUT` 整个 zip）
  - 现状（有意）：整包上传（云端文件 = 本地备份文件逐字节拷贝）——实现简单、无格式转换。包体积主要来自 `sessions/`（聊天历史累积）与正文本体（`data.db` 内的 `document_records`）。

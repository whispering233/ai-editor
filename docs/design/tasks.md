...
| [ ] 12.12 | **块编辑器中文化**（浏览器走查发现）：`useCreateBlockNote` 传 `dictionary: zh`（`@blocknote/core/locales`），否则 placeholder / 斜杠菜单 / 工具栏均为英文（与「语言: zh」不一致） | 12.5 | 单测 + 浏览器复看 placeholder = 「输入 '/' 以使用命令」 |

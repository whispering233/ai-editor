...
**批次十八（用户反馈七项，纯前端交互层，API/数据零改动）已完成并经逐批独立验证**：实体列表页去一级化残留、面包屑整站移除、`Ctrl/Cmd+S` 保存快捷键、新建即聚焦、中栏右下悬浮「问 AI」、focus 小条显示实体名称、设定树拖拽插入线强化；逐卡 commit 见 git log（A1/A2/A3 → B1/B2 → C1/C2 + 验证跟进修复）。

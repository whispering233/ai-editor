...
| [x] 12.7a | **服务端参考资料装载拆分**：`data.content` → `document_records`（单事务）；详情 join 回 `content`；列表摘要改读 `content_text`；purge 级联删行；reference data schema 瘦身 | 12.2 12.3 | 写一篇 → 直查 SQLite（`entities.data` 无 content、文档行有且投影正确）；列表 `content` 摘要来自投影；purge 后文档行消失 | ✅ `3bb6e57`（oracle PASS：真 HTTP + 直查 SQLite + 事务失败注入证明「同事务」；摘要路径 1 次 IN 查询、无 N+1） |
| [x] 12.7b | **文件机制退役（服务端/打包/云）**：删 `reference-files.ts` + `routes/reference.ts` + entity 路由写文件特例 + `trash.ts` file 分支 + backup `references/**`（白名单/打包/变更判定）+ cloud 并集前缀（只留 `sessions/`）+ shared `reference-file.ts`（frontmatter/sanitize）+ `REFERENCE_FILE_MISSING` | 12.7a | 全量测试；造项目确认备份 zip 条目 = 三文件 + `sessions/**`；本地 `test-project` 旧 `references/` 与旧 reference 行清理（人工，不入库） | **卡文修正（2026-10，卡 12.7a 裁决连带）**：`entity.ts` 的 kind/file 分支已随 12.7a 提前移除 ⇒ 本卡只剩「`reference-files.ts` + `routes/reference.ts` + `trash.ts` 文件分支 + 备份 `references/**`（白名单/打包/变更判定）+ cloud 并集前缀 + shared `reference-file.ts` + `REFERENCE_FILE_MISSING` 枚举」，另加：**存量行 PUT 后剔除遗留 `data.content` 旧键**（12.7a oracle 存疑项 2）、`test-project` 旧 `references/` 与旧 reference 行人工清理 | ✅ `b492524`（oracle PASS：真造项目 zip 条目 = 三文件 + `sessions/**`、老包严格 400 零半成品、存量键剔除实测、用例数 −18/−13 逐文件对账）；**test-project 旧数据已人工清理**（两本书共 7 条旧 reference 行 + 两个 `references/` 目录删除，不入库）；client 侧编译驱动最小收尾（1 文件/3 处，语义降级已登记到 12.8） |
| [x] 12.8 | **客户端参考资料页改造**：详情页编辑器换 `DocumentEditor`（BlockNote）+ 自动保存（debounce/flush/失败可见）+ 409 `DOCUMENT_STALE` 弹窗（重新加载 / 覆盖保存，参照 `pages/Manuscript.tsx`）+ 页头「导入 / 导出」复用 `lib/document-io`；列表页删「扫描」按钮与「未同步」提示条、来源列不再读 `kind`/`file_name`、草稿双路由（`new/md`、`new/link`）收敛为单一入口；「导入 md 新建」= 若文本以 frontmatter 块开头则取 `title` 作条目名、其余为正文（不解析 tags/category），无 frontmatter 用文件名（**替代 12.7b 删掉的 shared 工具，本地最小实现 + 单测**）；移除 `@uiw/react-md-editor` 依赖与样式引用；`lib/api.ts` 删 `scanReferences`/`getReferenceScanStatus`；`type` 缺省值由页面自带（REST 不兜底） | 12.7b 12.6 | UI 走查（编辑/保存/刷新一致、冲突弹窗、导入导出的有损提示、md 新建、旧草稿 hash 重定向）+ 全量测试 + 视觉守卫绿 | ✅ `70d4e04`（oracle PASS）；**浏览器走查 9/9 取证**：列表页只剩两个入口、旧草稿 hash 全部重定向、只填名称可建、块编辑器自动保存+刷新持久+切路由 flush、导出 JSON/md（有损先确认）、导入 md 新建（frontmatter title 作条目名且正文不含 frontmatter）、来源列只认 url、深浅两态可读 |

...
最近完成的批次：**桌面版（Electron 外壳）**——逐条事实见根 `CHANGELOG.md` 的 `## [v0.0.40]` 段（设计定稿 / pnpm 12 / 骨架打包 / 书库位置 / 目录选择 / 菜单与日志 / 安全导航 / 设置页通用 tab / 三平台发布链路）。

⚠ **待验证（本机 WSLg 无法断言 / 需真实环境）**：

1. 原生目录选择框的**可见性与交互**（WSLg 下 GTK 文件对话框挂起；最小 Electron 对照实验同样挂起，非本仓代码问题）——涉及首次启动选书库、书架页「浏览…」、设置页「更改…」后确认重启落在新书库三处；

...
- [ ] **T6b 原生 select → antd Select**（新增卡，由 T6 发现）：全仓 34 处 `<select>` / 9 文件 → antd `Select`。要点：`<optgroup>` → `options` 分组（`{label, options}`）、组级禁用 → option `disabled`、`value=""` 空态文案 → `placeholder`、表单 `name` 语义保持、密集行用 `size="small"`；完成后删 `selectClass`。判据：全仓 `<select` 为 0；下拉浮层观感与 antd 一致（portal + `{colors.surface-muted}` 选中面）；表单提交与受控值类型不变。**依赖 T6**。
...
- [ ] **T9 视觉纪律守卫**：新增单测（如 `packages/client/src/design-discipline.test.ts`）扫描源码禁止项——lucide 导入 / 硬编码色值（`#[0-9a-f]{3,6}`、`rgb(`、`hsl(` 白名单除外）/ `!` 前缀 antd 类 / 手写字号类 / **antd 组件根元素挂 `w-`/`h-`/`text-*` 等会被无层 antd CSS 压掉的类**；DESIGN.md 的 `designmd lint` 纳入验证步骤。判据：测试能对当前已收敛代码通过、对违规样例失败。

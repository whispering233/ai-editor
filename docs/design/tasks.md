...
最近完成的批次：**云端存档（v0.0.38）**——7 张卡 + 3 张补丁卡全部通过独立 oracle 验证（逐卡事实见 `CHANGELOG.md` 的 `## [v0.0.38]` 段）。

**开新卡**：从 `backlog.md` 选（当前剩余分两类）——

- **等触发条件**：并发推送串行化、失败重试退避、`head` 改 CAS 元数据、DELETE 受限云盘的清理收敛、restore 后 `backupStale` 短暂为真（未验证）、旧命名残留份的处理；

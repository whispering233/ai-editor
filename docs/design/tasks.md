...
- **卡序即依赖顺序**（1 → 2 → 3 → 4 → 5 → 6 → 7）；同一时间只允许一张卡处于实现态（避免多写者冲突）。
...
- **不改动范围外的东西**：`restore` 的覆盖语义、`project.json` 字段、SSE 契约、antd token 均不得借本批次改动（唯一例外：卡 1 的命名格式与卡 3 的 `backup-section` 拆分）。

| 卡 | 目标 | 主要交付物 | 完成判据（硬） |
| :--- | :--- | :--- | :--- |
| **1** | **备份命名升级**（设备段 + 统计段 + 类型中文化 + 兼容解析 + 目录 mtime 补强 + 列表显示设备/统计） | `shared`：`formatBackupFileName`（`device`/`stats` 参数）、`parseBackupFileName`（新格式一版 + 出参 `device?`/`stats?`）、`MAX_DEVICE_NAME_LENGTH`、`sanitizeDeviceName`（trim 1-16、禁 `-` 与保留字符）、`types/api.ts` 的 `BackupEntry` 加 `device?`/`stats?`；`db`：未软删人物/设定计数查询 + outline 非软删 `chapter` 计数；`server`：`writeBackup` 写入设备与统计、`hasFileChangesSince` 加两个打包目录自身 mtime；`client`：备份列表显示设备与统计（caption 行）、设置页备份标签输入与列表沿用 | ① 新备份文件名符合 `docs/api/20-api-backup.md` 格式（含设备与三段统计）；② 旧格式（秒级/带标签无类型段/`-m`/`-a`）仍可列出、恢复、参与保留策略；③ 删除 `references/`/`sessions/` 内文件后自动备份能被触发（目录 mtime 生效）；④ 单测覆盖命名格式矩阵（新 + 全部历史形态 + 标签含类似「人物32」字样不歧义）；⑤ `pnpm typecheck && pnpm lint && pnpm -r test` 全绿 + 新 commit |

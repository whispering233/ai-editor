...
- [ ] **T2 页头统一**：新建 `PageTitle` 薄壳（`Typography.Title level={4}`，20px/600）；替换 `<h1 className="text-xl font-semibold">`（Outline/HookPanel/Timeline 等）、`Typography.Title level={4}`（Trash/Settings/ReferenceList）、`level={5}` 区块标题用法统一；**附带清掉书名/项目名两处 `font-serif`**（`NavRail:102` 当前书名、`InfoBar:45` 项目名）与 `ui/dialog.tsx:169` 的静默失效 `font-heading`（T1 删掉 theme key 后已是 no-op）。判据：`grep -rn "<h1" src` 为空；页面标题来源唯一；界面 chrome 无 `font-serif` 残留（书封除外）。
...
- [ ] **T4 卡片与空态收敛**：`SectionCard` 内部换 antd `Card`、`EmptyState` 内部换 antd `Empty`（对外 props 不变，调用点零改动）；散落的手写 `rounded-xl border` 容器（约 15 处）逐一处置——是卡片则换 `Card`，是内衬小框则留 utility；**顺带清 4 处 `Tag color="blue"` preset 蓝**（`Settings:229`、`ReferenceList:469`、`EntityList:568/593`），改走 DESIGN.md 的灰底/`tint-*` 规则。判据：手写卡片容器归零（或逐处有理由）；`Empty`/`EmptyState` 视觉统一为虚线卡；全仓无 antd preset 色 Tag。

...
- **卡序即依赖顺序**（3 → 4 → 5 → 6 → 7；卡 1、卡 2 已完成并通过独立验证，见 `CHANGELOG.md` `Unreleased`）；同一时间只允许一张卡处于实现态（避免多写者冲突）。
...
| 卡 | 目标 | 主要交付物 | 完成判据（硬） |
| :--- | :--- | :--- | :--- |
| **3** | **设置页「备份」pane 重构 + 云端备份面板（账号配置）** | `client`：`components/settings/backup-section.tsx` 改为 sub-nav 容器（固定两项）、新增 `auto-backup-panel.tsx`（现有内容原样搬入）、新增 `cloud-backup-panel.tsx`（账号配置四输入 + 测试连接 + 保存 + 明文/配额说明）、`lib/api.ts` 云端点封装、`lib/cloud-config.ts`（表单↔补丁纯函数）、**页内 state**（不建 `stores/cloud.ts`：卡 3 只有单一消费者，store 上提留卡 6 的左栏按钮）；`docs/ui/DESIGN.md` 已登记（对照实现，不新增设计语言） | ① 设置页「备份」为左 160px 两项 sub-nav + 右侧面板，选中态 `surface-muted` 灰面（与 AI 模型页同构）；② 保存后重新拉取状态回显（密码字段保持为空 = 不修改）；③ 「测试连接」成功/失败均有明确反馈，禁用态与 loading 正确；④ 无「硬编码色值/内联样式/`!` 前缀类」，`design-discipline.test.ts` 与 `antd-tokens.test.ts` 通过；⑤ 浏览器核像素一次（浅/深两态）+ 新 commit；随卡落地：`POST /cloud/test` 不可达提示附 `cause.code`（已入 CHANGELOG） |
...
**卡 3 开工前必读（卡 2 oracle 复核提出，避免 UI 语义错）**：

1. **「清除凭据」= 清空 url + username**（密码框留空即可）——只清 url 不算清除（后端此时仍保留密码；「url 与 username 皆空 ⇒ password 一并丢弃」是服务端不变式）。
2. **三件套要么齐、要么全无**：url/username 为空时不应提交 password（后端会静默丢弃，用户会以为存上了）。
3. `GET /cloud/status` **当前只有 6 字段**（`configured`/`url`/`username`/`device`/`autoPush`/`projectId`）：store 不要按完整契约建 `remote`/`local`/`state`/`errorCode` 状态机（卡 4/5 才落地；`shared` 的 `CloudStatus` 类型已是收窄版，照类型写不会被骗）。
4. `POST /cloud/test` 的 `created:true` 可用于提示「已在云盘创建目录」；其 400（中文校验文案）与 502（`CLOUD_AUTH_FAILED`/`UNREACHABLE`/`QUOTA_EXCEEDED`）文案可直接展示，无需再映射。

...
最近完成的批次：**桌面版健壮性（v0.0.42）**——书库位置 3 级回退（含 OneDrive 跳过）+ 卸载时可选清除使用数据（含签名门禁）；事实见根 `CHANGELOG.md` 的 `## [v0.0.42]` 段。此前两批：v0.0.41（导入备份书名修正 + 打包口径收敛）、v0.0.40（桌面版整批）。
...
3. 原生目录选择框的**可见性与交互**（WSLg 下 GTK 文件对话框挂起，非本仓代码）——剩书架页「浏览…」与设置页「更改…」两处；
4. **macOS 的 Cmd+C/V**（菜单 Edit 角色，本机无法验证）、菜单项「打开书库/日志目录」的 `shell.openPath`。

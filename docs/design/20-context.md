...
| 聚焦上下文 | 请求携带的 focus（当前视图实体/节点/**推演节点集合**） | 由前端决定注入内容（推演节点集合只发布尔标记 `focus_deduction`，由服务端现读 `project.json` 展开），作为本轮消息的一部分；不做单独预算 |
...
- **推演节点集合的注入段（2026-09）**：只在大纲组页面（`#/outline` / `#/outline/:nodeId`）且项目存在推演节点时由悬浮「问 AI」触发，注入三段短文本——① 口径说明（单标记 = 开放式发散；多标记 = 相邻标记之间的剧情线探讨；明细可用 `get_deduction_marks` 获取）② **有序标记清单**（每标记一行：角色 · 章号 · 标题）③ **相邻区间摘要**（每段一行：跨越章数 · 已写章数）。**不注入**章摘要、区间内中间章清单与正文（正文按既定口径走只读工具分页拉取；区间明细走工具）——推演段只回答「作者把推演边界画在哪」，不预载剧情内容。
...
- **历史会话只回账目，不回占用百分比**：pi 的 `getContextUsage()` 建在 `estimateContextTokens(当前上下文消息)` 上，而该函数**未导出** ⇒ 历史重建等于复刻一套估算口径，故不做。历史会话的占用段按「窗口已知 / 占用未知」渲染（`? · 窗口`）：窗口由**服务端**在历史响应里带上（取当前激活模型目录，与 `/settings/llm` 同一条解析；无模型 → 省略该键 ⇒ 整段隐藏），占用百分比不重建；速度同理无值（时序不落盘）。

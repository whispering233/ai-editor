...
- **正文只读、按需拉取（2026-09）**：聚焦章时只注入「章标题 + 摘要 + 正文前 2000 字符（显式标注「节选」）」，完整正文由模型调只读工具按 `offset`/`max_chars`（默认 6000 字符）分页拉取，截断时告知续读起点。**不做整章自动入上下文、不做全书正文预载**——正文预算与对话历史共享同一个上下文窗口，预载会挤掉推理空间。

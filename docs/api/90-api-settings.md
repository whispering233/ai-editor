...
**配置所有权在本仓之外**：模型目录、凭据、运行参数全部落 pi agent dir（`~/.pi/agent/` 的 `auth.json` / `models.json` / `settings.json`），本仓只提供设置页的读写端点，**不维护第二份配置**。凭据解析顺序（pi 0.87.1，`pi-ai` `auth/resolve.js` + `auth/helpers.js`）：**auth.json 存量凭据优先**（一家一条；值支持字面 key / `$ENV_VAR` 引用 / `!命令`）→ 无条目时才回落到 provider 内置环境变量（如 `DEEPSEEK_API_KEY`，状态记为 `source: "environment"`）——因此不存在「两套来源竞争」，环境变量只是兜底。

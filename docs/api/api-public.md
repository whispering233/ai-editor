...
- 错误码统一枚举 `ErrorCode`（见 [error-code.md](./error-code.md)），REST / SSE `error` 事件 / 工具结果共用。
...
| `rel-` | 关系 |
| `proj-` | 项目 |
| `prop_`/`sess_`/`call_` | 运行时对象（提案/会话/工具调用） |
...
- 事件：`ping`（心跳）/ `tool_call` / `tool_result` / `text` / `proposal` / `done` / `error`；事件格式与生命周期见 [80-api-chat.md](./80-api-chat.md)。

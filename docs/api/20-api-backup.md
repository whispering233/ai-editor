...
> ```
>
> 毫秒时间戳（本地时区；字典序 = 时间序）＋ **类型段**（`自动` = 定时器 / 覆盖前快照，`手动` = 立即备份）＋ **设备段**（必填：来源机器，缺省 = 简化 hostname，设置页可改名；禁 `-`）＋可选**用户标签**（1-30 字符）＋ **尾部固定三段统计**（固定尾部使解析可从尾部倒切，标签里出现类似字样也不歧义，见下「设备与统计」）。**唯一格式（写入 = 解析）**：早期三类旧命名（秒级 `<YYYYMMDD-HHmmss>.zip`、带名称无类型段 `<YYYYMMDD-HHmmssSSS>-<名称>.zip`、单字母 `-m`/`-a` 段）**不再解析**——文件留在磁盘但不出现在列表、不可恢复、不参与保留策略。**升级兜底**：打开项目时若 `.backups/` 有文件但无一可解析 → 立即生成一份新格式备份（不重命名旧份：旧份无设备/统计信息，硬补会谎报）。格式与导出 zip 完全一致（三文件 + wal_checkpoint；**不含 `sessions/**`**——会话是纯本地目录，不进任何 zip；**正文与参考资料已随 `data.db` 进包，不再有 `references/` 目录条目**），**每项目保留最近 20 份**（超出删除最旧，含覆盖前自动快照）。全部端点要求当前项目已打开（无项目 → 409 `NO_PROJECT_OPEN`，与 `/config` 一致）。
>
> **设备与统计**：设备名规则 = trim 后 1-16 字符，**禁 `-`**（文件名段分隔符）与路径分隔符/保留字符（`\ / : * ? " < > |`）/控制字符/纯点，非法 → 400 `VALIDATION_ERROR`；生效值 = **云端配置的 `webdav.device` 优先**（见 [100-api-cloud.md](./100-api-cloud.md)），未配置或配置非法 → 简化 hostname（去域名后缀、滤非法字符、**剥首尾空白与 `_`**、截 16 字符，恒合法）。**设备名与备份标签都不得等于 WebDAV 应用密码**（两者都会进文件名并上传云盘，见 [`../design/40-cloud-sync.md`](../design/40-cloud-sync.md) §7；相等 → 400，存量坏设备名按未配置处理）。统计三项 = 备份时点该项目的**未软删**存量（人物数 / 设定数 / `outline.json` 中未软删 `chapter` 数，**不含回收站**），由服务端在生成 zip 时顺便统计并写进文件名；**UI 不自行解析文件名**（格式知识只活在 shared 纯函数里）。
>
> **与云端存档的关系**：云端存档推送的就是这里的一份备份文件（**逐字节拷贝、文件名原样**）；**删书**（本地 + 可选云端目录）走 `POST /project/delete`，具体见 [10-api-project.md](./10-api-project.md) 与 [100-api-cloud.md](./100-api-cloud.md)。
>
> **变更检测**：自动备份「有变更才备份」判定只看三文件 mtime（`project.json` / `outline.json` / `data.db` 及其 `-wal`，`data.db`/`-wal` 带 1s 容差）。**`sessions/` 不参与**（会话不进备份，纯聊天不应触发备份或云端推送）。**正文/参考资料的编辑与删除不需要额外探测**——它们写在 `data.db`（WAL 追加）里，`-wal` 变更判定已覆盖。
...
// Req（请求体可选；缺省 = 纯时间戳文件名）
{
  name?: string;  // 手动备份自定义名称：trim 后 1-30 字符（MAX_BACKUP_NAME_LENGTH）；
                  ...
                  // **不得等于已保存的 WebDAV 应用密码** → 400 VALIDATION_ERROR（标签会进文件名并上传云盘）
}
...
// Res: 200
{
  backup: {
    ...
    device: string;     // 来源设备（新格式必填；= 云端配置的设备名，未配置则 hostname 派生：去域名后缀、
                       //   滤非法字符、剥首尾空白与 `_`、截 16 字符——配置入口见 100-api-cloud.md）
    stats: { characters: number; settings: number; chapters: number };  // 生成时点的未软删存量
  };
}
```

**语义**：与自动备份同款管道（三文件 + wal_checkpoint → `.backups/<时间戳>-手动…>.zip` → 触发保留策略清理）；统计三项在生成 zip 时顺便算出并写进文件名。文件写入失败 → 500 `INTERNAL_ERROR`。**启用云存档且开自动推送时**：手动备份成功后自动推送这一份（见 [100-api-cloud.md](./100-api-cloud.md)）。
...
// Req
{
  ...
  name?: string;     // 新标签（规则同手动备份：trim 后 1-30 字符/禁路径分隔符与保留字符/禁纯点/自动剥 .zip；
                     //   非法或**等于已保存的 WebDAV 应用密码** → 400 VALIDATION_ERROR）。空串或缺省 = 清除标签段
}
...
**重命名只改标签段**（时间戳 / 类型 / 设备 / 统计保持原值——它们描述该备份的内容与来源，不能拿当前状态凑）。
...
**错误码**：400 `VALIDATION_ERROR`（文件名格式非法/新名称非法/新名称等于应用密码）、404 `VALIDATION_ERROR`（备份不存在）、409 `BACKUP_TARGET_EXISTS`（目标文件名已存在）。
...
**恢复流程**：
1. fileName 白名单校验（仅允许 `.backups/` 下的**唯一命名格式**；时间戳部分 ^$ 锚定纯数字 + 名称部分拒绝路径分隔符，防 `..` 穿越）
...
5. **会话目录不触碰**：`sessions/` 是纯本地目录（不进任何 zip）——恢复不写、不删、不覆盖它（存量旧包内的 `sessions/**` 条目接受并忽略）
6. 服务端当前项目引用不变（id 保留）；前端刷新 config/outline/会话数据

**错误码**：400 `VALIDATION_ERROR`（坏包/文件名非法）、404（备份不存在）、409 `SCHEMA_VERSION_MISMATCH`（同上）。

**与云端拉取的区别（不可混用语义）**：本端点是「回到那个时间点」= **三文件整体覆盖**（本地残留不混入）；云端 `POST /cloud/pull` 是「把另一台机器的东西拿过来」= 三文件覆盖——**两者都只覆盖三文件，都不碰 `sessions/`**（见 [100-api-cloud.md](./100-api-cloud.md)）。

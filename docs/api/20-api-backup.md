...
>
> **设备与统计**：设备名规则 = trim 后 1-16 字符，**禁 `-`**（文件名段分隔符）与路径分隔符/保留字符（`\ / : * ? " < > |`）/控制字符/纯点，非法 → 400 `VALIDATION_ERROR`；当前实现的来源 = 简化 hostname（去域名后缀、滤非法字符、**剥首尾空白与 `_`**、截 16 字符，恒合法；用户配置的设备名由云端批次接入后优先生效）。统计三项 = 备份时点该项目的**未软删**存量（人物数 / 设定数 / `outline.json` 中未软删 `chapter` 数，**不含回收站**），由服务端在生成 zip 时顺便统计并写进文件名；**UI 不自行解析文件名**（格式知识只活在 shared 纯函数里）。
>
...
// Res: 200
{
  backup: {
    ...
    device: string;     // 来源设备（新格式必填；当前实现 = 本机 hostname 派生的简化设备名：去域名后缀、
                       //   滤非法字符、剥首尾空白与 `_`、截 16 字符；用户在云端配置里改写的设备名由云端批次接入后优先生效）

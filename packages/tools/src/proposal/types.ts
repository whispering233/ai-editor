...
// - project_id：项目绑定（confirm/reject 校验归属）
// - references：被引用对象 id + updated_at 快照——实体/关系用自身 updated_at、
// 大纲节点用节点级 updated_at；确认（S7.5）时按 kind 重校验
...
export interface ProposalReference {
  ...
  id: string;
 /** 快照：实体/关系用自身 updated_at；大纲节点用节点级 updated_at */
  updated_at: string;
}
...
export interface Proposal {
 ...
 /** 项目绑定（confirm/reject 校验与当前项目一致） */
  project_id: string;
 /** 引用对象快照（存在性 + updated_at） */
 ...
 * 判定顺序：实体表优先（getEntity 已过滤软删），其次大纲树（存在且未软删）；
 * 两者皆不命中抛错（工具执行「抛错即失败」，executor 转结构化结果喂回 LLM 自纠）。
 * root 是树根而非端点，直接拒绝。
 */
 ...
 * **锚点仅章**：伏笔锚点（卡片 1.3：埋设 / 推进 / 回收）与变更记录锚点（卡片 1.5）
 * 一律为章——卷太粗、场景太碎（见 `docs/db/schema.md` 关系类型表、
 * `docs/design/10-data-model.md` §14.7）。
 * 与 REST 创建路径（`server/src/routes/relation.ts` / `server/src/routes/delta.ts` 校验）同口径；
 * **executor 直写 db 绕过 REST 校验**，故提案层与 executor 层都调本函数（双管）。
 *
 * @param label 错误文案前缀（默认「伏笔锚点」；变更记录锚点传入「变更记录锚点」）
 */
export function requireChapterNode(ctx: ToolContext, nodeId: string, label = "伏笔锚点"): OutlineFileNode {
  const node = requireOutlineNode(ctx, nodeId);
  if (node.type !== "chapter") {
    throw new Error(`${label}须为章（卷/场景不承载）: ${nodeId}`);
  }
  return node;
}
...
/** 提案工具入口统一中止检查（长工具执行中检查 signal；提案生成是同步短操作，入口检查一次即可） */

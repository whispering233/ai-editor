...
describe("reorder_timepoints", () => {
  it("正常重排：按新序重写 sort_order 0..n-1，全部时间点 updated_at 刷新（全量变化语义），返回 { reordered: n }", () => {
    const ids = seedTimepoints(["第二天黄昏", "少年时", "大婚之夜", "结局"]);
  ...
  it("参数防御：非数组 / 空数组 / 非字符串元素 → 抛错（防脏调用写脏数据）", () => {
    ...
    expect(() => executeReorderTimepoints(makeCtx(), makeProposal("propose_reorder_timepoints", { timepoint_ids: "tp-x" }))).toThrow(
      /执行参数缺失或非法: timepoint_ids/,
    );
    expect(() => executeReorderTimepoints(makeCtx(), makeProposal("propose_reorder_timepoints", { timepoint_ids: [] }))).toThrow(
      /执行参数缺失或非法: timepoint_ids/,
    );
    expect(() =>
      executeReorderTimepoints(makeCtx(), makeProposal("propose_reorder_timepoints", { timepoint_ids: [ids[0], 42] })),
    ).toThrow(/执行参数缺失或非法: timepoint_ids/);
    expect(listTimepoints(db).map((r) => r.id)).toEqual(ids); // 零副作用
  });
});

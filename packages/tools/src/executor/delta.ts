...
// - **character 已移除字段兜底（卡片 5.4 / 5.5）**：经 `assertRemovedCharacterFields` 拒 character 的
// `status`/`abilities`（手工构造 proposal 才可达）——守卫与白名单均与提案层同源（shared 常量）
...
import { assertFactFieldsSetOnly, assertRemovedCharacterFields } from "../proposal/delta.js";
...
export const executeAddDelta: ExecutorFn = (ctx, proposal) => {
 ...
 // character 已移除字段兜底（卡片 5.4 / 5.5）：同源守卫，补齐两层同口径
  assertRemovedCharacterFields(targetType, changes);

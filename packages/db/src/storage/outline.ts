...
//
// 严格三层（卷→章→场景）、节点携带 updated_at 版本戳、顶层 schema_version、
// 软删字段 deleted/deleted_at。
 ...
 * 读取 outline.json。
 *
 * 缺失文件语义（本项目首次初始化即创建）：**返回最小空树**而非抛错——
 * 调用方（server 打开项目前的探测、初始化流程）无需为「文件还不存在」分支处理；
 * 空树的 schema_version 取当前 SCHEMA_VERSION，与初始化流程写入的一致。
 * 注意：返回的是新对象，调用方修改它不会影响后续读取。
 *
 * JSON 损坏 / 结构不符：**抛错**，不静默重建——静默覆盖会掩盖文件损坏并可能
 * 丢弃用户数据，由上层决定提示用户还是重建。
 * schema_version 数值与 SCHEMA_VERSION 不一致时**不在此校验**（以
...
function validateOutlineFile(parsed: unknown): OutlineFileTree {
  if (
    ...
    throw new Error("outline.json 顶层结构不符（需 {id:\"root\", type:\"root\", schema_version, children[]}）");
  }
  return parsed as OutlineFileTree;
}

/**
 * 原子写 outline.json（临时文件 + fsync + rename，见 writeJsonAtomic）。
 * 顶层 schema_version 随 tree.schema_version 原样写入——保持与 project.json
 * 同步写入是调用方职责（两个文件用同一版本号常量）。
 * 节点 updated_at 版本戳由调用方在变更后通过 touch/update 更新，
 * 本函数不隐式修改树。
 */
...
/**
 * 更新节点版本戳（任何字段变更——含 children 重排、软删/还原——
 * 由服务端原子写时统一更新 updated_at，支撑 提案快照比对）。
 ...
 * （title/summary；data——与 title/summary 同等对待，patch 含 data 时整体替换，

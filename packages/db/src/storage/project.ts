...
/** project.json 文件名（项目根目录） */
export const PROJECT_FILE_NAME = "project.json";

/** 文件名（项目规则文件，项目根目录，与 project.json 同级） */
 ...
 * （首次初始化时自动创建），null 即未初始化，由上层决定创建流程；
 ...
 * 结构演进判定依据，id/name 是初始化即写入的必填项；language/prompt/
...
function validateProjectFile(parsed: unknown): ProjectFileConfig {
  if (
    ...
    throw new Error("project.json 顶层结构不符（需 {id:string, name:string, schema_version:number, ...}）");
  }
 ...
 * 读取项目规则文件 内容。
 *
 * 缺失文件语义：**返回 null**—— 是可选文件（新项目/未迁移项目可能没有），

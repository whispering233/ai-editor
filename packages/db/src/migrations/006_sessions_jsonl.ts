...
//
// 安全性/幂等性：
// - 导出是「按表内容整文件重写」（writeLegacySessionFile 原子写，输出仅由入参决定）——文件写不可回滚，
...
import type { LegacySessionRow } from "./legacy-sessions.js";
import { isValidLegacySessionId, writeLegacySessionFile } from "./legacy-sessions.js";
 ...
 */
function readLegacyRowsBySession(db: Db): Map<string, LegacySessionRow[]> {
  const rows = db
    .prepare(
  ...
  const bySession = new Map<string, LegacySessionRow[]>();
  for (const r of rows) {
    const list = bySession.get(r.session_id);
    const row: LegacySessionRow = {
      id: r.id,
      role: r.role,
      content: r.content,
...
const migration006: Migration = {
 ...
 // 旧库无 chat_messages（新库从未建过该表 / 已迁移过）→ 无事可做（不因缺表中断迁移链）
    ...
    const exportable = [...bySession.keys()].filter((id) => isValidLegacySessionId(id));
    ...
    for (const [sessionId, rows] of bySession) {
      const targetId = isValidLegacySessionId(sessionId) ? sessionId : legacySessionId(sessionId);
      if (targetId !== sessionId) remapped += 1;
      writeLegacySessionFile(ctx.projectRoot, targetId, rows);
    }
    if (remapped > 0) {
      console.warn(

// @whispering233/ai-editor-db 表结构声明（决策 49，批次十五 15.1）
//
// 单一事实来源：doc/database/schema.md（列名/类型/CHECK/索引逐字对照）。
...
export const entities = sqliteTable(
  "entities",
  {
    ...
    // data 列保持 text 模式（决策 49：坏 JSON 防御在行映射层，不用 drizzle json mode）
    data: text("data").notNull().default("{}"),
    // 时间轴线性序（决策 26 + G2 修订）：event 与 timepoint 各类型内线性（各自 0..n-1），其余类型恒为 NULL
...
export const relationRecords = sqliteTable(
  "relation_records",
  {
    ...
    metadata: text("metadata"), // JSON 扩展元数据（text 模式，决策 49）
    created_at: text("created_at").notNull(),
    updated_at: text("updated_at").notNull(),
    deleted_at: text("deleted_at"),
  },
  (t) => [
    // 3 个部分索引（决策 12 修订），仅覆盖未软删行
...
export const deltaRecords = sqliteTable(
  "delta_records",
  {
    ...
    target_type: text("target_type").notNull(),
    target_id: text("target_id").notNull(),
    changes: text("changes").notNull(), // JSON: [{field, op, from?, to?, value?}]（text 模式，决策 49）
...
export const chatMessages = sqliteTable(
  "chat_messages",
  {
    ...
    project_id: text("project_id").notNull(), // 会话按项目隔离（决策 18 修订）
    role: text("role").notNull(),
    content: text("content"),
    tool_calls: text("tool_calls"), // JSON（text 模式，决策 49）

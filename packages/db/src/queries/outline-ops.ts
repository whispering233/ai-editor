 ...
 * @param data 节点结构化信息（可选；校验由路由层按层级 schema 执行，本模块仅透传存储）
...
export function updateOutlineNodeInfo(
 ...
 // data 浅合并：仅合并传入字段，现有字段保留（与 updateEntity 同语义）
...
/** 递归还原子树（子节点若仍在回收站则一并还原），返回还原的子节点数 */
...
export function restoreOutlineNode(dir: string, nodeId: string, updatedAt: string): { children: number } {
 ...
 // 祖先链校验：路径 [root, ..., nodeId] 中排除 root 与自身的中间祖先

// @whispering233/ai-editor-db schema 演进：删库重建（S1.1）+ 未来版本拒绝+ 增量迁移
//
// 删库重建」）与修订（2026-08）——
// - 以 data.db 的 PRAGMA user_version 为准判定版本；三态分流：
// - user_version === SCHEMA_VERSION → 正常打开
// - user_version > SCHEMA_VERSION（未来版本）→ **拒绝打开**（提示升级程序，
// 不触发任何重建/备份/写操作，数据原封不动）——堵「装新版后回退旧版 → 降级清零」
// - user_version < SCHEMA_VERSION（旧版本）→ **有迁移路径**（MIGRATIONS 存在
// 从当前版本到目标版本的连续链）→ runMigrations 前向迁移（数据保全，迁移前
// 时间戳快照）；**无迁移路径** → 删库重建兜底（备份 v{n}.bak 留档）
...
export interface MigrationResult {
 /** 是否发生了删库重建 */
  rebuilt: boolean;
 /** 是否发生了前向迁移（有迁移路径的旧版本经 runMigrations 升级） */
  migrated?: boolean;
...
export function ensureSchemaCompatible(
 ...
 // 未来版本拒绝打开（先关连接防句柄泄漏，再抛错；数据文件零触碰）
  if (current > SCHEMA_VERSION) {
    closeDatabase(db);
 ...
 * 只写版本号；**不重建、不备份、不碰 outline.json**（旧逻辑会把有内容的大纲重置为空树）*/
 ...
 // ora-3 S2：迁移中途失败 → 关闭本次打开的连接（对比 拒绝分支与重建分支
...
/**
 * 迁移前快照 data.db → `data.db.v{n}.{YYYYMMDDHHmmssSSSZ}.bak`：
 ...
 * **project.json 不动**：其 schema_version 仅用于 JSON 结构判断，id/name 跨启动稳定。
 *
 * **崩溃窗口（MVP 可接受）**：步骤 4（删除旧库）与步骤 5（重建空库）之间进程崩溃 →
 * 磁盘上无 data.db（下次 open 自动重建空库），且旧库备份已留档不丢数据；
 * 但同版本号多次重建会覆盖旧备份（命名 v{n}.bak 不带时间戳，的
...
export function rebuildProjectStorage(db: Db, dir: string, dbPath: string, oldVersion: number): RebuildOutput {
 ...
 // 6. 重置 outline.json 为最小空树（顶层 schema_version 同步写入）
...
/**
 * 备份 data.db 主文件 → `data.db.v{n}.bak`（备份带版本号、不覆盖旧备份）。
 *
...
/**
 * 备份 outline.json → `outline.json.v{n}.bak`（重建时同步重置并留档）。
 *

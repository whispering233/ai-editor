...
//
// 历史 ~6000；usage 基线——优先最近真实 usage，裁剪历史后重置基线防预算漂移）、
// （提示词三层注入：内核 + 项目 + 临时）、（POST /api/v1/chat 的
...
/** 分层 token 预算（系统 ~500 / 聚焦 ~3000 / 历史 ~6000；可整体或单项覆盖）。
 ...
 */
 ...
 /** 工具清单（默认内部 listTools()；测试可注入 mock） */
  tools?: ToolListEntry[];
 /**
 * 最近一次成功响应的 usage 基线（本层触发裁剪后重置为 null）。
 ...
 * **回写（S7.3 接线）**：本层消费的 lastUsage 是「历史段」口径（见
 ...
 * 4. usage 基线：未触发裁剪时保留 lastUsage（真实用量优先）；一旦触发裁剪即重置为
...
export function buildContext(input: BuildContextInput): AssembledContext {
 ...
 // 预算判定优先真实 usage 基线；一旦判定超限触发裁剪，基线即重置

...
// ToolDispatcher 接口与消费，测试用 mock，S7.4 提供真实现）。
...
/** 轮次上限（8 轮；持续 tool_call 死循环兜底） */
export const DEFAULT_MAX_ROUNDS = 8;

/** 单轮总预算 ms（120s，含 LLM 重试退避与工具执行；测试可覆盖） */
export const DEFAULT_ROUND_TIMEOUT_MS = 120_000;

/**
 * 单次 attempt 超时 ms（「LLM 单次 attempt 另有自身 fetch 超时」落地——
...
/** 提案载荷（S7.4 工具调度层构造 preview；对齐 proposal 事件 data 形态） */
export interface ProposalPayload {
  proposal_id: string;
 /** 提案类型 = 产生它的 propose_* 工具名（data.type） */
  type: string;
 ...
 * - proposal：恒在对应 tool_result 之后、循环继续之前发送（顺序约定）
 * - done：自然终止（无工具调用），携带 session_id
 * - error：终止（模型最终失败 / 8 轮上限 / 超时 / token 超限 / 调度器缺陷）或
 * 用户取消（aborted=true）；S7.6 收到 error 后立即关闭流
 */
 ...
 * 防止 tool_call ↔ tool_result 错位配对）
 * - signal：「工具执行中检查取消」（S7.4 executor 承担；长工具执行须监听）
 * - 本层不抛错（失败编码进 isError 结果）；抛错视为调度器缺陷 → 循环终止。
 * **例外：AbortedError（tools analysis/utils，取消语义）→ 原样抛穿**——
 ...
 * ****：必须把 onEvent 转发给 chatStream 的 onEvent——runAgent 依赖它累积文本与
 * 收集工具调用（失败轮的重试会自动丢弃旧累积，无需调用方处理）；失败按 chatStream
 * resolve 出 { ok:false, ... }，不 throw。
 ...
 * **硬（ora S7.3 审核 S3）**：回调抛错不逃逸——run 侧统一兜底吞掉（防 runAgent
...
export interface RunAgentInput {
 ...
 /** 聚焦上下文文本（聚焦层；S7.6 查询实体/大纲节点后拼好传入） */
  focus?: string;
 /** 项目提示词（项目层） */
 ...
 /** 轮次上限（默认 8；测试可覆盖） */
  maxRounds?: number;
 /** 单轮总预算 ms（默认 120s 含重试退避；测试可覆盖） */
  roundTimeoutMs?: number;
 /** 单次 attempt 超时 ms（默认 60s——LLM 单次调用独立超时，与轮次总预算分离；测试可覆盖） */
 ...
 * 或模型错误码 / 调度器缺陷码） */
 ...
 /** 实际执行轮数（≤ maxRounds） */
  rounds: number;
 /**
 * 重试累计（重试与轮次分开计量、不互相消耗；ora S7.3 审核 S6 断言口径——
...
const LENGTH_TRUNCATION_NOTICE = "finish_reason=length：参数可能不完整，不执行";
...
/**
 * 主循环重试分类：可重试 = llm 传输类（429/5xx/网络断开/流截断）∪ attempt 超时；
 * abort（ABORTED 码）由 classifyLLMError 判为不可重试（取消不重试）。
 */
 ...
 * 主循环只处理一种形态。同时实现「超时信号与用户取消分离」：
 * attempt 超时（timedOut=true 且非用户取消）→ 归一为**可重试** ATTEMPT_TIMEOUT；
 * 用户取消 → aborted 原样（不重试）。
 * 非 abort 异常照原样抛出（防御路径——chatStream 本不 throw，交 withRetry 分类）。
 */
...
/**
 * S7.2 回写历史段真实用量换算（口径统一，ora S7.3 审核 S1——三处注释等价表述，
...
/** LLMToolCallResult（解析后形态）→ wire 请求形态（assistant.tool_calls 回填原始 arguments 串—— 配对依赖 id） */
...
export async function runAgent(input: RunAgentInput): Promise<RunAgentResult> {
 ...
 // 本轮文本/工具调用累积（attempt 级变量——重试自动丢弃失败轮累积，半条 assistant 不重发）
    ...
    const attempt = async (): Promise<ChatStreamResult> => {
      ...
      attemptText = "";
      attemptCalls = [];
 // 独立 AbortController：超时 abort 与用户取消分离——勿把 AbortSignal.timeout
 // 直挂用户取消链路（会把超时误判为取消、吞掉可重试机会）。
 // attempt 超时 = min(剩余轮次预算, attemptTimeoutMs)：单次调用有自身超时（
 // 「LLM 单次 attempt 另有自身 fetch 超时」），轮内重试仍受 roundDeadline 兜底
      ...
      try {
 // 失败轮重试复用原 payload（半条 assistant 绝不追加）
 ...
 // 可重试分类 + 轮次预算兜底（预算耗尽即不再重试，最终以超时错误终止）
    ...
    try {
      result = await withRetry(attempt, {
        isRetryable,
        signal, // 用户取消不重试；退避 sleep 即时中断（llm/retry.ts 承担）
        maxRetries,
 ...
 // ---- 5. 模型调用最终失败 → error 事件终止（最终失败以 error 呈现，不静默） ----
    if (!result.ok) {
 // attempt 超时耗尽重试后终止：错误码映射为 ErrorCode 的 AGENT_TIMEOUT（SSE 事件）
 ...
 // 注：retries 报告值**不清零**（ora S7.3 审核 S6 断言口径）——「成功即清零」
 ...
 // ---- 7. 无工具调用 → done 终止（done 携带 session_id） ----
    if (calls.length === 0) {
 ...
 // ---- 9. 调度前检查轮次预算（ora S7.3 审核 M2：「单轮总预算含工具执行」——
 ...
 // 截断/解析失败的调用：合成失败结果（LLMToolCallResult.error 已含说明，含 length 口径）
    ...
    if (toDispatch.length > 0) {
      ...
      try {
        dispatched = await deps.dispatcher(toDispatch, signal); // 工具执行中检查取消（S7.4 承担）
      } catch (err) {
 // 取消语义传播（S7.4 约定）：executor 在工具执行中检查 signal，
 // 命中取消抛 AbortedError（tools analysis/utils）——此处识别并按**用户取消**终止
 // （aborted=true、不重试、不喂回），不得落入调度器缺陷分支（否则取消会被误报为
 // AGENT_DISPATCH_ERROR 且 aborted=false，与「取消不重试」语义冲突）

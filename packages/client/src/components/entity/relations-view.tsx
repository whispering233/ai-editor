...
//   端点名为四类实体时点击跳详情 #/entities/:type/:id，outline_node 灰显不可点。
...
/** 端点名（含徽标）：四类实体可点击跳详情；outline_node 灰显不可点 */
function EndpointLink({ type, id, name }: { type: string; id: string; name?: string }) {
  const label = name ?? id;
  const clickable = (ENTITY_TYPES as readonly string[]).includes(type);
  if (!clickable) {
    return (
  ...
  return (
    <span className="flex min-w-0 items-center gap-1.5">
      <button
        type="button"
        onClick={() => navigate(`/entities/${type}/${id}`)}
        title={`打开《${label}》`}
...
export function RelationsView({
  ...
  // 拉全量关系（depth=1 双向紧邻；进入 tab 挂载即拉，外部 reloadKey / 内部 tick 变化重拉）
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError(null);

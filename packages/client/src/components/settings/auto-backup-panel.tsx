// 设置页「备份 → 自动备份」面板（B2 + B2.5 + B2.6；卡 3 从 backup-section 拆出）
//
// 挂载点 = `backup-section.tsx`（sub-nav 容器：自动备份 / 云端备份）；本文件只负责自动备份面板：
//
...
export function AutoBackupPanel() {
  ...
  async function handleBackupNow() {
    ...
    try {
      ...
      await createProjectBackup(name.length > 0 ? name : undefined); // 空输入不传 name
  ...
  return (
    <div>
      ...
      {/* 加载强确认（「关键交互」：展示备份时间 + 类型标签 + 名称 + 大小 + 覆盖说明 +

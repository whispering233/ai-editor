// Base UI ContextMenu 护栏（批次十：右键菜单替代行级 AskAiButton）
// 同 dropdown-menu （Base UI error #31）：ContextMenuLabel（= Menu.GroupLabel）**必须**由
...
// GroupLabel 本体命中同一（hooks 在 SSR 同样执行）——同 chat-panel.test.tsx 既有护栏模式。
...
describe("Base UI ContextMenu （error #31 根因护栏，）", () => {
  it("裸 ContextMenuLabel（无 Group）→ 抛 MenuGroupContext is missing（#31 ）", () => {
 ...
 // 行级右键菜单结构合法性（RowContextMenu 同构）；打开态崩溃由上面两个用例覆盖
    expect(() =>
      renderToString(
        <ContextMenuRoot>

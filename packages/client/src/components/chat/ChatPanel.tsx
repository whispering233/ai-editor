...
import { buildDeductionMarks, formatRelativeTime } from "@whispering233/ai-editor-shared";
 ...
 */
export function focusLabel(ctx: FocusContext, name?: string | null, deductionCount?: number): string {
  if (ctx.focus_deduction === true) {
    return deductionCount === undefined ? "推演节点" : `推演节点 · ${deductionCount} 个`;
  }
 ...
 * 仅一项「删除会话」（antd danger 样式由 token 派发）；禁用条件 = `streaming`（在途生成，
 * 服务端 409 `SESSION_BUSY` 兜底）。
  ...
  return (
    <div className="flex h-12 shrink-0 items-center gap-1 border-b border-border px-2.5">
      ...
      {deleteTarget !== null && (
        <ConfirmDialog
          title="删除会话"
          description={`将删除会话「${sessionTitle(deleteTarget)}」及其 ${deleteTarget.messageCount} 条消息，删除后无法恢复。`}
          confirmLabel="确认删除"
          danger
...
function FocusBar() {
  ...
  const deductionCount = useMemo(
    () =>
      outline === null || config === null
        ? undefined
        : buildDeductionMarks(outline, config.deductionNodes).length,
    [outline, config],
  );
  // 推演形态无 targetId（只发布尔标记）⇒ 下面对 names/resolve 的解析自然跳过（不请求、不闪 id）
  ...
  return (
    <div className="flex shrink-0 items-center gap-1.5 border-t border-border bg-muted px-3 py-1.5">
      ...
      >
        正在讨论：{focusLabel(focusContext, resolvedName, deductionCount)}
      </Tag>
    </div>
  );
}
...
/**
 * 底部区（focus 小条 + 输入区）：导出供 SSR 走查（zustand 的 getServerSnapshot 恒为初始态，

...
describe("BACKUP_FREQUENCY_OPTIONS（修订：关闭 + 1/5/10/15/30/60，与 shared 对齐）", () => {
  ...
  it("label 格式：每 N 分钟", () => {
    expect(BACKUP_FREQUENCY_OPTIONS[1]).toEqual({ value: 1, label: "每 1 分钟" }); // 新增档位
...
describe("formatBackupMeta（行元信息：设备 · 规模）", () => {
  ...
  it("设备 + 三项统计 → 中间点分隔（唯一命名格式保证两项恒存在）", () => {

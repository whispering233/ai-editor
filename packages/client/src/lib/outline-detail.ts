// 大纲节点详情页辅助纯函数与配置（S12.2；outline.json「节点结构化信息 data」节
...
/** 场景冲突层次 → 中文（麦基冲突三层次；checkbox 组标签） */
...
/**
 * 按节点层级的 data 字段配置（麦基字段集；root 无 data 不适用——详情页仅卷/章/场可达）：
...
/** 场景节点选择器选项（树序遍历 + 仅 scene 叶子；不含卷/章——引用字段只指向场景）。

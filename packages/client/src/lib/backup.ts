...
/**
 * 备份时间展示（同书架日期格式，settings.md 线框「08-13 10:15」）：
 * 当年 → `MM-DD HH:mm`；跨年 → `YY-MM-DD HH:mm`；非法输入原样返回
...
export function formatBackupTime(iso: string, now: Date = new Date()): string {
  ...
  if (sameYear) return `${pad(date.getMonth() + 1)}-${pad(date.getDate())} ${hhmm}`;
  return `${String(date.getFullYear()).slice(-2)}-${pad(date.getMonth() + 1)}-${pad(date.getDate())} ${hhmm}`;
}

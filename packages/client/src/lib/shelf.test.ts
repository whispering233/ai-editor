...
import { PROJECT_ORIGINS } from "@whispering233/ai-editor-shared";
...
// 非缺省出处的取值不手抄字面量：取值域的单一定义 = shared `PROJECT_ORIGINS`
const OTHER_ORIGIN = PROJECT_ORIGINS.find((origin) => origin !== "book")!;

describe("groupShelfBooks", () => {
  it("固定组序：交错传入也按组归位", () => {
    const groups = groupShelfBooks([
      book("拆1", OTHER_ORIGIN),
      book("书1", "book"),
      book("拆2", OTHER_ORIGIN),
    ]);
    expect(groups.map((g) => g.origin)).toEqual(["book", OTHER_ORIGIN]);
  ...
  it("组内保持传入顺序（倒序排序归服务端，前端不再排）", () => {
    expect(
      groupShelfBooks([book("b2", "book"), book("b1", "book")])[0].books.map((b) => b.id),
    ).toEqual(["b2", "b1"]);
  });

  it("空组不渲染：只有一类书 → 只返回一组（只有一类书时就是一张列表）", () => {
    expect(groupShelfBooks([book("书1", "book")]).map((g) => g.origin)).toEqual(["book"]);
    expect(groupShelfBooks([book("拆1", OTHER_ORIGIN)]).map((g) => g.origin)).toEqual([
      OTHER_ORIGIN,
    ]);
  });

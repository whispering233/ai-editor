// 「设为当前位置」唯一提交实现（卡片 1.1：详情页按钮 + 大纲行右键菜单共用）
//
// 语义：PUT /project/config { current_position }——store updateConfig 成功后自动重拉 config，
// 联动 InfoBar「当前位置」/ 大纲行尾徽标 / 概览页 / compute 预览默认节点 / 伏笔健康指标基准。
...
/**
 * 节点层级是否可承载「当前位置」（卡片 1.1 章级收窄：仅章）——卷太粗、场景太碎，
...
/** 设为当前位置；成功 → 「已设为当前位置」，失败 → 泛化错误提示（节点已删/服务端拒绝/网络） */

...
export default function Timeline() {
  ...
  // ============ 行内编辑（点击事件名/时间点名行内编辑，替代原「编辑/重命名」按钮） ============
  ...
  async function handleEditEventName(id: string, name: string) {
    try {
      ...
      throw err; // 失败 rethrow——组件 catch 保持编辑态 + 保留输入值（可修正后重试）
    }
  }
  ...
  return (
    <section className="flex h-full min-h-0 flex-col">
      {/* 页头（统一壳）：标题 + 控件行（左=标签筛选；右=AI 排序/新建事件/新建时间点）+ 分割线；
      ...
      <Dialog open={tpCreateOpen} onOpenChange={(v) => !v && setTpCreateOpen(false)}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button onClick={() => setTpCreateOpen(false)} disabled={tpSubmitting}>
              取消
            </Button>
            <Button
              type="primary"
              htmlType="submit"
              form="create-timepoint-form"

...
// ============ query_relationships（关系图子图，「关系查询」） ============
...
// ============ get_delta_history（Delta 历史，「状态查询」） ============
...
// ============ 提案类工具（「提案类（需确认）」+ 「工具扩展」提案类，S6.6） ============
//
// 语义（「返回语义」2026-08 修订）：AI **不能直接修改数据**——propose_* 仅产出
// 提案对象（proposal_id + 一句话摘要），tool_result 不含预览细节（避免 LLM 误以为提案已生效
// 而重复提案）；完整预览只经 SSE proposal 事件推送 GUI（S7 实现）。
// 参数签名与 逐字对齐（propose_add_relation(source, target, type, metadata?) 等）：
// 端点 id（source/target/plant_at_node_id 等）由服务端在生成时自动识别实体/大纲节点类型
// 并采集 updated_at 快照，故入参只需 id 字符串。
...
// === propose_create_hook（创建伏笔提案，「工具扩展」提案类） ===
...
// === propose_reorder_timepoints（时间轴时间点重排提案，G2，注记） ===
...
// === propose_create_reference（创建参考资料提案） ===

// 备份文件名工具（.backups/ 时间戳命名 + 类型标签 + 重命名）
// 纯函数，零 Node 依赖（client 浏览器打包安全）
// 时区约定：文件名时间戳为本地时区（无时区后缀），format/parse 对称使用本地时间
//
// 格式——<YYYYMMDD-HHmmssSSS>[-<kind>][-<名称>].zip，kind 段单字母 m/a：
...
// 兼容解析（不迁移，历史备份不重命名）：
// - 旧秒级：<YYYYMMDD-HHmmss>.zip → kind=auto（毫秒为 0）
// - 旧带名称无 kind 段：<YYYYMMDD-HHmmssSSS>-<名称>.zip → kind=manual

import { MAX_BACKUP_NAME_LENGTH, type BackupKind } from "../constants/backup.js";

/** 新格式：<YYYYMMDD-HHmmssSSS>[-<kind>][-<名称>].zip——组 8 = kind 段（a→auto、m→manual、
...
/** 备份文件名解析结果（time + 类型 + 可选自定义名称） */
export interface ParsedBackupFileName {
 /** 本地时区时间（文件名时间戳；旧秒级格式 → 毫秒为 0） */
  time: Date;
 /** 备份类型（由文件名 kind 段解析推断：auto / manual） */
  kind: BackupKind;
 /** 手动备份自定义名称（自动备份/快照/旧秒级备份无此字段） */
  name?: string;
}
...
export function parseBackupFileName(fileName: string): ParsedBackupFileName | null {
 ...
 // （无 kind 段 → 兼容为 manual）。歧义注：旧「名称恰为单字母 a/m」的备份按新格式
 ...
 * 毫秒精度；文件名时间戳命名，`.backups/` 内按文件名排序即时间序（保留策略依赖）。
 *

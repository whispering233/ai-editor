...
// 提案所属项目 ≠ 当前项目 → 409 PROPOSAL_PROJECT_MISMATCH（get 返回 null 后
// peek 仍可见 ⇒ 跨项目误操作；防御性——正常流程切换项目时提案已被清空）
// 快照重校验失败 → 409 PROPOSAL_STALE（引用对象不存在 **或** updated_at 不一致，
// 任一失败即 STALE；前端提示重新生成提案）
// 执行失败 → 500 INTERNAL_ERROR（未定义该错误码——执行失败属服务端状态异常/数据竞争，
// 前端按通用错误呈现）
//
...
//
// 提案移除语义（一次性消费，「提案是毫秒级交互对象、无跨会话恢复」落地）：
...
/** 快照校验失败统一映射 409 PROPOSAL_STALE（存在性 + updated_at 任一失败即 STALE） */
...
/**
 * 取当前项目提案（confirm/reject 共用，项目绑定）：
...
proposalRoutes.post("/:proposalId/confirm", (c) => {
  ...
  try {
 // 快照重校验（存在性 + updated_at 比对，任一失败 409 PROPOSAL_STALE）；
 ...
 // 执行失败（幂等冲突/数据竞争/防御校验抛错）：未定义该错误码，按 500 内部错误呈现；

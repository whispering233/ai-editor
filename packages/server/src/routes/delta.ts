...
//
// （可见性联动）、S13.3 收紧（变更目标仅实体类型——大纲节点不可作为变更目标，
 ...
 * db 层 insertDelta / computeState 均不校验触发节点存在性（未定义该错误码，delta.ts 注释），
 ...
 * 仅创建路径拒绝）。
 * **排除 event（C2 oracle 审查口径）**：event（时间轴事件）不产生 Delta——
 * **本收紧覆盖 REST 创建路径**（API 直连）：client 目标下拉已过滤（delta-create.ts），
 * REST 层同步拒绝（不产生「死 Delta」：无字段清单可编辑、client 无管理入口）；
 * AI 提案通道（propose_add_delta）在 tools 层独立拒绝（proposal/delta.ts，同）。
 * 已存在的 event 目标历史数据仍按实体端点展示（db 层 ENTITY_TARGET_TYPES 已含 event）。

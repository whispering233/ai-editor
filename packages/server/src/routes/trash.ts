...
// 写序：**先 DB 后 JSON**——先级联还原 relation/delta，再原子写 outline.json。
...
trashRoutes.post("/outline/:nodeId/restore", (c) => {
 ...
 // 0. 祖先链预校验：存在软删祖先 → 409，且不产生任何 DB 副作用
...
// 写序：**先 DB 后 JSON**——先物理清除 relation/delta，再原子写 outline.json

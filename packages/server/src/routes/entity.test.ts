// 实体路由测试（S3.3）：列表/创建/详情/更新/软删 + 校验 + event 时间轴（C2）
// 覆盖：type 过滤与摘要（camelCase）、分页/排序、软删过滤、创建 201 与按类型 data 校验、
// 部分更新（浅合并）、404 映射、级联计数、详情 relations 紧邻（S3.2 接入）、
// event 泛型 CRUD（C2）+ move 端点 + occurs_in 关系链路
...
describe("GET /api/v1/entity/:type 列表", () => {
 ...
 // 摘要：setting 暴露 tags（rules 前 3 个）
 ...
 // 非 setting 类型不带 parent 字段（仅 setting）
    await createCharacter(app, "张三");
 ...
 // 非 setting 类型忽略 parent_id（仅 setting）
    await createCharacter(app, "张三");
...
describe("event 时间轴（C2）", () => {
 ...
 /** 事件列表 id 序（恒按 sort_order 升序——服务端） */
...
describe("timepoint 时间轴（G2）", () => {
 ...
 // move_to 目标 = 当前挂载 → 幂等跳过重建挂载（只重排，关系 id 不变）
  ...
  it("move_to：timepoint_id=null 移到未挂载区——旧挂载移除、仅重排", async () => {
    openProject();
    const app = buildApp();
    const tp0 = (await createTimepoint(app, "拂晓")).id;
    const e0 = (await createEvent(app, "事件0")).id;
...
describe("设定 move（同级重排 / 改父 + 重排复合写）", () => {
 ...
 // 防环：当前链 B→C→A，A 挂 B → A 成为 B 的祖先链成员 → 400

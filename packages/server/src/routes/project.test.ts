...
function initProjectDir(dir: string, config: ProjectFileConfig, outline: OutlineFileTree = makeOutlineTree()): void {
  ...
  setUserVersion(db, SCHEMA_VERSION); // 版本匹配（以 user_version 为准）
  closeDatabase(db);
}
...
describe("POST /project/create", () => {
 ...
 // data.db：user_version 已写（S1.1 审核建议：避免 open 时无意义重建）
  ...
  it("config 覆盖参数生效：name/language 写入 project.json 且 updated_at 刷新（prompt 已废弃，strict 拒绝）", async () => {
    ...
    expect(config?.prompt).toBeUndefined(); // 新写入不再产生 prompt 字段
    expect(config?.updated_at).not.toBe(T0);
  });

  it("create config 传 prompt → 400 VALIDATION_ERROR（prompt 废弃，strict schema 拒绝）", async () => {
    const dir = makeTmpDir();
    const res = await buildApp().request("/api/v1/project/create", {
      method: "POST",
      headers: HOST_HEADERS,
  ...
  it("相对路径 → 400 INVALID_PROJECT_PATH（要求绝对路径，拒绝 cwd 折叠歧义）", async () => {
    const res = await buildApp().request("/api/v1/project/create", {
      method: "POST",
      headers: HOST_HEADERS,
...
describe("POST /project/open", () => {
 ...
 // 手工建 v1 结构库（旧 entities DDL：无 sort_order、CHECK 4 种）+ 数据 + user_version=1
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
 // 有迁移路径（MIGRATIONS 002）的旧版本经前向迁移打开——migrated 附加提示，非删库重建
...
describe("GET/PUT /project/config", () => {
 ...
 // 盘上同步（写入 project.json）
  ...
  it("PUT 传 prompt → 400 VALIDATION_ERROR（prompt 废弃，strict schema 拒绝）", async () => {
    const dir = makeTmpDir();
    const app = await openProject(dir);
    const res = await app.request("/api/v1/project/config", {
      method: "PUT",
  ...
  it("PUT current_position 指向软删节点 → 400 OUTLINE_NODE_NOT_FOUND", async () => {
    const dir = makeTmpDir();
    const app = await openProject(dir);
    const res = await app.request("/api/v1/project/config", {
      method: "PUT",
...
describe(" 自动迁移", () => {
 ...
 // prompt 为空（makeConfig 无 prompt）→ 不创建 
...
describe("GET /project/list（书架：创作根 books/ 扫描）", () => {
 /** 在创作根下造一本书（books/<name>/ 含 project.json，updated_at 可控） */
  ...
  it("books/ 下两本书 → 返回两条（id/name/path/updatedAt），按 updatedAt 倒序", async () => {
    const root = makeTmpDir();
    setProjectRoot(root);
    ...
    expect(body.data.books).toEqual([
      { id: "proj-第二本", name: "第二本", path: join(root, "books", "第二本"), updatedAt: "2026-08-02T10:00:00Z" },
      { id: "proj-第一本", name: "第一本", path: join(root, "books", "第一本"), updatedAt: "2026-08-01T10:00:00Z" },
    ]);
  });
  ...
  it("非书目录（无 project.json）与普通文件被过滤", async () => {
    ...
    const body = await res.json();
    expect(body.data.books).toEqual([
      { id: "proj-真书", name: "真书", path: join(root, "books", "真书"), updatedAt: "2026-08-01T10:00:00Z" },
    ]);
  });
 ...
 // 根自身不出现在 books 列表（books 只列 books/ 子目录）
    expect(body.data.books).toEqual([
      { id: "proj-书架上的书", name: "书架上的书", path: join(root, "books", "书架上的书"), updatedAt: "2026-08-01T10:00:00Z" },
    ]);
  });
 ...
 // list 扫到该书：id/name/path/updatedAt 与盘上 project.json 一致（不依赖 open/currentProject）
 ...
 // 项目身份按 id 回传（不按 name）
    expect(body.data.books).toEqual([
      { id: readProjectFile(bookDir)?.id, name: "联动书", path: bookDir, updatedAt: readProjectFile(bookDir)?.updated_at },
    ]);
...
describe("GET /project/export（zip 导出三文件）", () => {
 ...
 // data.db 能查到 → 证明 wal_checkpoint(TRUNCATE) 真实合并（卖点）。
 ...
 // 解包出的 data.db（导出时已 checkpoint）应能直接查到 WAL 中的实体
  ...
  it("zip 不含任何 key 内容（key 存用户级配置，天然不入包）", async () => {
    const dir = makeTmpDir();
    const app = await openSeededProject(dir);
    const res = await app.request("/api/v1/project/export", { headers: HOST_HEADERS });
    const unzipped = unzipSync(new Uint8Array(await res.arrayBuffer()));
...
// ============ POST /api/v1/project/import（校验 + 原子搬入） ============

describe("POST /project/import（zip 导入新书）", () => {
 ...
 // name 归一为目录名（id 是身份、name 是展示名——「目录名 = 书名」不变式）
 ...
 // 拒绝导入：books/ 无残留
  ...
  it("旧版本备份（user_version < SCHEMA_VERSION）→ 409 文案提示「旧版本程序且无可用迁移路径」（当前 MIGRATIONS 为空无路径，仍拒绝）", async () => {
    const root = makeTmpDir();
    setProjectRoot(root);
...
// ============ POST /api/v1/project/backup/rename（B2.6） ============
...
// ============ 导出/导入 zip 条目（ora-4 钉死） ============

describe("导出/导入 zip 条目（ora-4）", () => {

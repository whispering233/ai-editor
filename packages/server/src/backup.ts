...
/**
 * 保留策略：每项目保留最近 MAX_BACKUPS_PER_PROJECT 份，超出删除最旧。
 * 排序依据 = 文件名时间戳（可解析的才参与；非法文件名不参与保留判定）。
 * 清理失败不阻塞调用方（记日志即可，「清理失败不阻塞备份主流程」）。
 */
...
/**
 * 带预算的 zip 解压（ora-4 zip 炸弹防御，自 import 提取）：
...
/** project.json 顶层最小校验（shared 无文件形态 schema，不扩范围） */
...
/** outline.json 顶层校验（与 db 包 validateOutlineFile 同构：root 根 + schema_version 数字 + children 数组） */
 ...
 * - v < SCHEMA_VERSION → **有迁移路径** → 接受（open/restore 后自动前向迁移）；
 * **无迁移路径** → 409（文案标注版本过旧无路径）
 * - v > SCHEMA_VERSION → 409（备份来自更高版本程序）
...
export function validateBackupPackage(zipData: Uint8Array): { entries: Record<string, Uint8Array>; projectId: string } {
 ...
 // project.json 顶层（JSON 可解析 + id/name/schema_version）
    ...
    if (!isValidProjectFile(projectConfig)) {
      throw new HttpError(400, "VALIDATION_ERROR", "备份包内 project.json 顶层不符（需 id/name/schema_version）");
    }

 // outline.json 顶层（{id:"root",type:"root",schema_version,children[]}）
    try {
      const outline = JSON.parse(readFileSync(join(tmpDir, OUTLINE_FILE_NAME), "utf8"));
      if (!isValidOutlineFile(outline)) {
        throw new HttpError(400, "VALIDATION_ERROR", "备份包内 outline.json 顶层不符（需 {id:root,type:root,schema_version,children[]}）");
      }
    } catch (err) {
      if (err instanceof HttpError) throw err;
 ...
 * （overwriteProjectFiles 同步 config 后——向无 的项目恢复含遗留 prompt 的旧备份时，
 * 内存 config 有 prompt 但 未创建，chat 读 为 null，恢复的 prompt 在下次
 * open 前不注入）。**幂等**（prompt 非空且无 才迁移），**失败不阻塞**（记录日志，
 * 下次 open 重试——「迁移在 open 流程内完成，失败不阻塞打开」）。
 *
...
export function migratePromptToAgents(project: ProjectContext): void {
  ...
  if (readAgentsFile(project.root) !== null) return; // 已有 不覆盖（一次性）
  try {
 ...
 * 3. 重连 + 版本对齐（v < 当前且有迁移路径 → 前向迁移，v === 当前 → 原样。
 * 校验阶段已拒绝 v > 当前与无路径旧版，此处不会触发重建/拒绝）
 * 4. 同步内存 config（刚原子写入，readProjectFile 必非 null；损坏抛错由 catch 恢复连接）
 * 5. migratePromptToAgents（恢复的 project.json 含遗留 prompt 且无 → 迁移）
...
export function overwriteProjectFiles(
 ...
 // （oracle 评审修复）：restore/import 覆盖路径同步触发 迁移——
 // 向无 的项目恢复含遗留 prompt 的旧备份时，内存 config 有 prompt 但 
 // 未创建，chat 读 为 null，恢复的 prompt 在下次 open 前不注入。
 // migratePromptToAgents 幂等（prompt 非空且无 才迁移），失败不阻塞覆盖。
...
/**
 * 为**未打开的书**生成覆盖前快照（import 覆盖书架其他书场景，「覆盖前自动快照」）：
 ...
 * v < 有路径时此处前向迁移）→ project.config 同步新 project.json
 * 5. 项目 id 保留（当前项目引用不变）；重启定时器（频率可能随备份变化）
 *

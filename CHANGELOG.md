...
> **拆解提速 / 批大小实测版本**：拆解 S2 改为「N 段并行、段内串行」（创作根配置 `decompose.concurrency`，缺省段数在代码常量），样本书（48 章 / 25 万字）真跑 25/25 批、零失败、墙钟 981s；关系端点判据唯一化（S2 不再预丢，交 S3 悬空过滤）；批大小加双向预算守卫；批大小四档实测判定「保持」（全文一批仅作小体量「快速概览」）。回归：build / typecheck / lint / `-r test` 全绿（199 文件 / 3020 测试）+ 样本书并发真跑 + 批大小四档实测 + 每卡独立验证与最终集成复核。

...
- **配色改造（视觉变更）**：⑧ **用户标签 tint 收到 3 档**（`#cce9fc` sky 淡蓝 / `#d1e2c4` sage 浅黄绿 / `#ffd800` yellow 亮黄，取代原 6 档马卡龙；中途的 5 档里 `ice` 与 `sky` 只差明度、`mint` 是唯一中间绿且观感偏墨绿 → 两者删除）；⑤ **类型/分类徽标改描边式**（`{colors.surface-muted}` 底 + **1px `#d94a4a` 边框** + 随主题翻转的墨字）；③ **人物页左栏「角色定位」改橙字**（浅色 `#b4551f` 4.93:1 / 深色 `#ef8354` 6.24:1——原橙作文字压白底只有 2.61:1，12px 小字不可用）。
...
- `docs/ui/DESIGN.md`：登记 tint 五色新值 + 色相分布事实（sky/ice 同色相只差明度）；新增 `type-badge-border`（`#d94a4a`，两态同值）与 `Character Role` 两态值 + 对比度实测（tint 三档：底 vs 白 1.26–1.39 / 墨字 8.80–9.71 / 深色 5.96–6.18）；`type-badge` 规格改为「**描边式**（浅底 + 1px 彩色边框 + 墨字）」并记录「彩色边框压在灰底上要 ≥3:1、浅色系只能做面积不能做线」与 error 同色相的已知代价；`character-rail` 行描述同步。

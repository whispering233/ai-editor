...
> **UI 收口**：关联总览列对齐统一、大纲行尾徽标不再推移删除按钮、人物页进度节点下拉可搜索；**人物关系星形图整体移除**（分组列表已是关系网的完整明细，图的索引价值不足以抵一张 SVG）；**标签 tint 准入收口**（tint 只给用户标签，枚举类型/分类徽标改中性）。
...
- **配色改造（视觉变更）**：⑧ **用户标签 tint 收到 3 档**（`#93c5fd` sky 冷蓝 / `#d1e2c4` sage 浅黄绿 / `#ffd800` yellow 亮黄，取代原 6 档马卡龙；中途的 5 档里 `ice` 与 `sky` 只差明度、`mint` 是唯一中间绿且观感偏墨绿 → 两者删除）；⑤ **类型/分类徽标改描边式**（`{colors.surface-muted}` 底 + **1px `#d94a4a` 边框** + 随主题翻转的墨字）；③ **人物页左栏「角色定位」改橙字**（浅色 `#b4551f` 4.93:1 / 深色 `#ef8354` 6.24:1——原橙作文字压白底只有 2.61:1，12px 小字不可用）。
  - **描边式怎么选出来的**（实测）：彩色边框压在**灰底**上需 ≥3:1 才看得见——橙 `#ef8354` 对 `#f0eeec` 仅 1.43:1、浅红 `#ffb3b3` 仅 1.07:1（1px 线宽下等于没画）；`#d94a4a` 是少数两态都过线的红系（浅底 3.61 / 深底 3.34）。实底彩色已否决：橙底上墨字 4.70:1 偏紧、白字 2.61:1 不可用，且高频行首摆一块告警色太吵。底/字回到随主题翻转（`bg-accent` + `text-foreground`：浅 10.59:1、深 8.96:1）——不再需要“两态恒定”那一套（那是为了不透明橙底才存在的）。
  - **深色模式已验算**：tint 三色均为浅色调，沿用「同色相 20% 不透明度叠深色面板 + 81% 白字」后对比度 6.08–6.66 全部达标（零新增深色值）；浏览器实测徽标两态：深色 `rgba(255,255,255,.055)` 底 + 81% 白字 + 红边界，浅色 `#f0eeec` 底 + 墨字 + 同一红边界。
  - **注意**：tint 由 6 档改 3 档 ⇒ `hash % 3`，**所有既有标签会重排换色**（同名仍恒同色）；描边式徽标比实底高/宽 2px（1px 边框）。
  - **已知代价**：徽标边框色与语义 `error`（`#e03131`）同色相（描边比实底轻、位置固定在行首/类型列；已登记）。
...
- `docs/ui/DESIGN.md`：登记 tint 五色新值 + 色相分布事实（sky/ice 同色相只差明度）；新增 `type-badge-border`（`#d94a4a`，两态同值）与 `Character Role` 两态值 + 对比度实测（tint 三档：底 vs 白 1.36–1.80 / 墨字 6.80–8.98 / 深色 6.08–6.66）；`type-badge` 规格改为「**描边式**（浅底 + 1px 彩色边框 + 墨字）」并记录「彩色边框压在灰底上要 ≥3:1、浅色系只能做面积不能做线」与 error 同色相的已知代价；`character-rail` 行描述同步。

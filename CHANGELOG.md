...
- **服务端口分段隔离**：web 生产（默认 3500 起）/ web 开发态（严格单端口）/ 桌面端（**刻意不动**，偏好锚在起点）各自独占端口窗口、互不相交——端口分段唯一定义 = shared `constants/ports.ts` 的 `PORT_RANGES`（server / `vite.config` / desktop 全部派生，守卫测试断言窗口两两不相交），兜底 +1 不再跨形态游走（此前 web 生产与桌面端共用同一起点分段，`pnpm dev` 的 Vite proxy 可能静默打到桌面端 server）。⚠ web 生产默认端口变更：旧书签需更新，web 形态的浏览器偏好（主题 / 面板 / 写作面）会重置一次；桌面端不受影响。
...
- **「小说拆解」整功能移除**：删掉 `/api/v1/decompose/*` 全部端点、`#/decompose` 进度页与书架「拆解小说」入口、拆解会话的只读态与在跑禁删守卫；server 侧的拆解管线（切分 / 批执行 / 归并 / 报告）与 shared 拆解契约（端点 schema、`Decompose*` 类型、`ERROR_CODES.SESSION_READONLY`、会话 id 前缀常量）一并删除；`scripts/decompose-batch-experiment.mjs` 实测工具与 server 侧「LLM 调用只走 pi Agent 路径」的源码扫描守卫同批移除（该纪律本身保留为文字约定）。
- **拆解数据层回退**：`decompose_jobs` / `decompose_batches` 两表随迁移 v11 **DROP**（`SCHEMA_VERSION` 10 → 11，幂等、纯 DDL）；`project.json` 的 `origin` 字段废止（存量键读侧容忍、不再写；书架不再分组，当前书仍按 id 判定）；创作根 `.ai-editor/config.json` 的 `decompose` 段废止（存量键读侧忽略）。
- **理由与路径消失**：超长文本无论怎么工程优化都绕不开上下文窗口爆满 / 腐化 / 漂移，产出物的可信度无法自证；「拿 txt 导入既有小说并自动分章建书」这条路径随功能一起消失，**无替代**。
...
- **拆解小说（导入式批量管线）**：书架页新增「拆解小说」入口——选小说 txt → 预览切分（编码探测 / 章数 / 字数分布 / 警告 / 预估）→ 开始拆解 → 自动建档（项目 + 卷章大纲 + 章正文全量导入）→ 逐章 LLM 抽取 → 归并（别名合并 / 关系聚合）→ 拆解报告；全流程在 `#/decompose` 进度页可见。
  - **章节切分**：GB18030 / UTF-16 编码探测（不装依赖）、规则融合 + 5 维结构性评分、相邻重复标题合并、超长块回扫、卷首目录页启发式、无章节时按字数等分兜底；预览只读并给出警告（编号重启 / 疑似合并章 / 目录页丢弃 / 退化等分）。
  - **拆解范围与成本可控**：可选起止章（正文仍全量导入），开始前给出批次数 / 调用次数 / 粗估费用（费率读模型目录，不自建定价表）。
  - **进度页**：阶段条 / 进度条 / 批次列表（可展开看该批抽取结果）/ 单批重跑（二次确认）/ 中止与续拆 / 完成总结卡；概览页新增拆解任务卡片，书架当前书行显示进度徽标。
  - **断点续跑（同一 job 的未完成批）**：job 状态与批结果落 `data.db`（迁移 v9，新增 `decompose_jobs` / `decompose_batches` 两表，随备份/导出/云自动携带）；切书自动暂停、服务重启后归一为「已暂停，可续拆」；单批重跑幂等（不覆盖用户手工编辑、消失项软删/物理删）。
  - **入口形态**：原始字节上传（不在客户端解码），选文件 → 预览 → 填书名 → 开始拆解，一条路径浏览器/桌面通用。
  - **拆解会话可查（每轮一枚落盘）**：每轮拆解在项目 `sessions/` 落一枚会话（会话 id `decompose-<jobId>`，S2 各批 + 归并 + 报告都写进它）；chat 面板可见但**只读**（输入框不渲染、`POST /chat` 拒该前缀、在跑 job 时禁删）；每批一个独立 turn（`resetLeaf` 新根 + 关闭自动压缩，避免历史累积导致 O(N²) 重复付费）；`#/decompose` 新增「拆解记录」时间线（批开始 / 每次尝试失败原因 / 批完成计数与用量 / 归并 / 报告 / 快照组成），保留最近 5 枚、超出按最旧清理并在新会话里显式告知。
  - **续拆（同一项目拆剩下的章）**：完成态「继续拆解」→ 预览（已拆章标注 / 范围 / 预估，**不吃源文件**——正文已全量导入）→ 开新 job；已拆 = 历史全部 job 的 `done` 批覆盖的章并集，缺省范围 = 未拆章最小覆盖区间；旧 job 与批结果全保留。
...
- **全站 `Ctrl/Cmd + S` = 保存 + 本地存档**：任意页面按 `Ctrl/Cmd + S` 先执行本页保存动作（章正文页 = flush 待存内容并立即落盘；行内编辑态 = 先提交该行），保存成功后再生成一份手动备份（与「立即备份」同管道）。距上次快捷键存档不足 `SHORTCUT_BACKUP_THROTTLE_MINUTES` 时只保存不存档（连按不会刷掉保留窗口、不会连推云端）；保存失败不存档，存档失败单独提示；浏览器原生「保存网页」对话框被拦下。
- **设置页「快捷键」tab**：二级 tab 末位新增纯说明页，列出当前支持的快捷键（按系统平台显示 `⌘` / `Ctrl`）。键位引用真实绑定用的常量，键位改了清单跟着改。

### Changed

- **README 精简到 99 行（原 183 行）**：① 删掉「发布与安装」里失准的历史版本说明段（当前版本号、缺发布 tag 清单、npm 实际序列、逐版本要点——原段只列 4 个缺发布 tag，实测 13 个）；② 「当前能力」里的逐版本叙事（v0.0.32~v0.0.51）删除，改为能力域清单 + 指向根 `CHANGELOG.md`；③ 「打包安装」「发布与安装」两段合并为指向 `docs/design/build.md` 的指引（用户安装命令保留）；④ 调试配置示例收为指向 `docs/design/config.md`。保留：技术栈表、三栏布局与正文书写面、包结构、桌面版安装表与自动更新注意、快速开始命令、文档地图、设计原则。
...
- **正文写作面升级：写作工具条 / 专注模式 / 写作偏好（字体·字号·行高·纸张·缩进）**：
  - **常显写作工具条**：章正文与参考资料正文的编辑器上方多了一行常驻工具条——块类型下拉、加粗 / 斜体 / 下划线 / 删除线、文字颜色、对齐、缩进 / 取消缩进、添加链接、撤销 / 重做，不用再猜 `/` 命令（选中文字时的浮动工具条保留）。滚到章末时它随正文一起离开，不会一直霸着屏幕。
  - **写作设置**：正文可切换字体（无衬线 / 宋体 / 楷体 / 仿宋 / 等宽）、字号（14 / 16 / 18 / 20）、行高（1.5 / 1.8 / 2.0），还有三种纸色（默认 / 米黄 / 暖灰）、两种纸纹理（横线 / 网格，默认关闭、纯装饰不对齐文字）与**段首缩进两字符**开关。选择记在本机（所有正文共用），刷新后保持；代码块固定等宽、界面文字不受字体偏好影响。
  - **专注模式（仅章正文页）**：一键收起左栏、右栏、信息条与页头，正文独占整窗口，字数与保存态移到工具条右端；再点一次或按 `Esc` 退出（关闭下拉/弹窗的那次 `Esc` 不算），离开本章自动还原。
  - **命令帮助**：工具条的「命令帮助」弹窗列出各工具条入口、编辑器内置命令与快捷键（快捷键按当前系统自动显示 `Ctrl` / `⌘`）。
  - **保存时间**：保存成功后页头显示「已保存 · 12:03」（专注模式下显示在工具条右端）。
- **正文可手动保存**：章正文页与参考资料详情页（编辑态）的工具条右端都有「保存」，点一次即落盘（平时停止输入约 1.5s 会自己保存）；一次点击恒发一次保存，所以「已保存 · 时间」总会刷新——即便没改动也看得到反应。参考资料页原先在表单底部的「保存」按钮收进工具条，与章正文页同形同位；**新建**参考资料仍是底部的「取消 / 创建」。
- **写作面铺满屏幕下方（正文页 + 参考资料页）**：之前正文只有几行时，可写区域会缩成一小块、下面大片灰底（看着像坏了）；现在白纸始终铺满剩余高度，**点最后一行下方的空白即把光标落到文末**。长文照旧滚动，不会被压缩。
- **章正文页页头常驻**：滚动正文时章标题、字数/保存态与「上一章 / 下一章 / 导入 / 导出」一直留在顶部（与参考资料页一致）；保存失败提示也不会被滚走。

### Changed

- **大纲页会记住你的视图**：切到「章视图」后进章正文、切到别的页、刷新都会保持在章视图（写作期不必每次重选）；只记视图，「全部折叠」仍是一次性状态。
- **章视图「写正文」改为行尾图标按钮**：原先是段下划线文字，现在是行尾的铅笔图标（悬停提示「写正文」），与页内其余行尾控件同形；行为不变，点击仍然跳到该章正文页。
- **参考资料「分类」有了统一的徽标形态**：分类此前在列表里是裸文字、在详情页头又是另一套灰徽标，看着既不像标签也不像类型；现在两处都是同一枚描边式徽标（与「标签」的彩底 chip 形态区分），分类中文名（素材摘抄 / 灵感记录 / 写作理论 / 设定参考）照旧回显。
- **关联页的类型徽标挪到了名称前面**：原先徽标排在名称末尾，名称一长一短，相邻两行的徽标就一前一后对不齐；现在源列、目标列各自成一列对齐（人物页关系区与实体详情页的「A 关系 B」读法保持原样）。

## [v0.0.49] - 2026-09-19

### Added

- **正文进系统（章级正文 + 所见即所得编辑器）**：每章有了属于自己的正文——从大纲页章视图或章详情页点「写正文」进入编辑器，块级结构（标题 / 列表 / 引用 / 代码 / 表格 / 图片等）所见即所得；停止输入片刻自动保存（页头显示「N 字 已保存」），离开页面立即落盘，保存失败有可见提示与重试。同一章在两处同时打开编辑时，后保存方会看到冲突提示，可选「重新加载」或「覆盖保存」。章正文可导出 / 导入：块 JSON 无损、Markdown 有损（有损时先提示并需确认）。
- **大纲页章视图显示正文字数与「写正文」入口**：章视图每行显示该章已写字数（未写过不显示），并可直接跳到该章正文页。
- **参考资料正文改用块编辑器 + 单文件导入导出**：参考资料详情页换成与章正文同一套块编辑器（自动保存、冲突提示同款），新建只需填名称；列表页不再有「扫描」与「未同步」；参考资料改为以单个文件导入 / 导出（Markdown 有损时先确认，块 JSON 无损）。
- **AI 可读正文（只读）**：AI 现在能读到章正文（按需分页拉取，长章分段读完并告知续读位置）与参考资料全文，用于分析节奏、核对与设定的冲突、评价具体段落；对着某一章提问时，AI 会自动带上该章开头的一段正文作为开场上下文（不整章灌入）。工具面**不存在任何正文 / 文档写工具**——AI 只能读、评论与建议，落笔由作者完成。
- **大纲页自动编号与「章视图」（2026-09）**：树视图的卷/章行改为 `第N卷` / `第N章` 编号徽标（仍走 `TypeChip`：`min-w-14` + `tabular-nums`；场景行仍为「场」）；页头「全部折叠」左侧新增视图切换按钮（`大纲树` ⇄ `章视图`，文案 = 目标视图；页面 state 不持久化，章视图隐藏「全部折叠」）；新增**章视图 = 平铺章列表**（按阅读序，行 = `第N卷` + `第N章` + 标题 + 摘要 + 伏笔标记 + 「阅读进度」徽标；单击标题就地改名、双击进详情，改名态下双击不跳详情——与树视图同守卫）。
...
- **删除大纲节点时的级联范围修正**：此前删除场景或章，会误把**父级**的变更记录（Delta）与关联关系一并软删，同时漏掉其下级节点的记录；现在删除严格按「自身 + 全部后代」级联，删除与还原的范围一致。

### Changed

- **备份包内容**：备份 zip 不再包含 `references/` 目录——内容 = 项目三文件（`data.db` / `outline.json` / `project.json`）+ `sessions/`（对话历史）；云端存档的并集合并同样只认 `sessions/`。

### Removed

- **参考资料的磁盘文件与扫描机制**：`references/` 目录（Markdown 文件 + frontmatter）、「扫描」入口与旧的文件式编辑器（Markdown 源码 + 实时预览）一并下线——参考资料正文与其它创作数据同库同生命周期，随备份 / 导出 / 云同步走同一套机制。旧项目里的 `references/` 目录不再被读取或打包，也不自动删除（属用户文件）。

...
- **标签筛选（决策 31，批次五 J3，仅设定）**：排序行旁「标签」**可搜索下拉**（批次八 O1 起：输入关键词过滤候选，保留「全部」重置；聚合既有 `data.tags` 标签集，K2 修订后 tags 为分类统一字段），选后 `GET /entity/setting?tag=<标签>` 与服务端组合过滤（走 db `matchDataFilters.tags` 内部管道），与搜索/排序/分页组合；切换类型/tab 重置。
- **上级设定筛选（决策 32 + 批次八 O1 修订，2026-08，仅设定）**：排序行旁「上级设定」**可搜索下拉**（候选 = 全部设定按名称排序 +「全部」重置项；输入关键词过滤候选；标签筛选并列），选后 `GET /entity/setting?parent_id=<id>` 服务端**递归子树**过滤——匹配该上级的**所有后代**（直接或间接属于，不含上级自身，决策 30 belongs_to 层级、决策 32），与搜索/标签/排序/分页组合（AND）；当前 tab 列表行点击父 chip（M2）与设定树（I4）职责不变；切换类型/tab 重置；选中父下无设定 → 空态文案「《…》下暂无设定」；已选父在候选中不可见（软删/超 200 截断）时兑底「（已删除或不可见）」。
...
- **空态（三种，按命中条件优先展示；2026-08 决策 32 后扩为三支）**：
  - 该类型无任何实体：「还没有人物，新建一个」+ [新建]。
  - 搜索无结果：「没有匹配「xxx」的实体」+ [清空搜索]。
  - 上级设定筛选无结果：「《X》下暂无设定（含其所有层级的设定）」+ [清除上级筛选]（决策 32）。
- **加载态**：列表骨架。
...
| 工具栏 | **「全部展开 / 全部折叠」按钮**（批次八 O5，2026-08；全部折叠 = 仅保留根级；与大纲页工具栏同款，border 文字按钮，H4） |

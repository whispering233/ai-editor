...
- **时间点组块**（= 时间标签）：**组标题** = 大圆点 + 时间点名（`text-sm font-medium text-foreground`，F4 样式）+ 事件计数 + [重命名]（行内编辑，`PUT /entity/timepoint/:id` name）+ [+ 在此时间点新建事件]（H4：与重命名同排横向展示，带边框；自动挂载该时间点）+ [移入回收站]（垃圾桶图标按钮，H2：直接软删不弹确认，`DELETE /entity/timepoint/:id`）+ 折叠按钮（`aria-expanded`，折叠后仅标题行、轴线仍连续）；**组内事件堆叠**（各自不再画线，轴线容器级贯穿）。
...
- **时间点内新建**：时间点标题行「[+ 在此时间点新建事件]」→ 对话框：`name` + `description` + `tags`（逗号分隔输入，F8 建议）+ 可选关联节点选择器 → `POST /entity/event` + `POST /relation`（occurs_at 挂载该时间点）。

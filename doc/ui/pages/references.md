...
> 契约来源：`doc/design/decisions.md` 决策 36（第 7 种实体类型 reference）+ 决策 43（两类承载：本地 md 文件 + 外源链接）+ **决策 44（分类自定义，批次十二）**、`doc/database/schema.md`（reference data 字段）、`doc/api/endpoints.md`（entity CRUD + scan 端点）、`doc/api/tools.md`（search_references / propose_create_reference）。
...
- 筛选行：分类 ▾ 下拉（**全部分类 + 项目内已用分类**，聚合现有条目 `summary.type` 去重，显示存量回显名——决策 44；原生 select，分类数量少 YAGNI 不做可搜索下拉）+ 标签 ▾ 下拉（聚合现有 tags 集合 + 全部，复用决策 31 标签筛选管道 + N2 可搜索下拉模式）+ 搜索输入（关键词，标题命中）+ 分页。
- 列表：**表格平铺（批次十二 R3）**——`thead`（标题/分类/标签/来源四列表头，`bg-muted/50 text-xs text-muted-foreground/70` 样式对齐 EntityList）+ 单行 `tr`（`border-b border-border/50 hover:bg-muted`，行高从两行收为一行）：
  - 标题列：`font-medium` 截断，**点击 = 行内编辑**（Enter 确认、Esc 取消——决策 37/38 模式）；
...
- **外源链接详情页（kind='link'）**：标题（行内编辑同 md）+ **URL（必填，校验非空 + 格式建议 http(s)）** + 分类输入（同决策 44 文本框 + datalist）+ 标签编辑 + 内容 textarea（可选备注/摘录）+ **建立关联按钮**（2026-08 用户补充：外源链接详情页同样需要）+ 删除按钮。
...
- 分类显示（决策 44 修订）：**无预置枚举色**——存量分类回显名（TYPE_LABELS 映射：material→素材摘抄 等，仅存量显示、非可选建议），新自定义分类原样显示字符串；统一 `text-muted-foreground` 徽标（`rounded-md border border-border bg-muted px-1.5 py-0.5 text-[11px]`），文字标签区分，不用色块（色弱友好）。

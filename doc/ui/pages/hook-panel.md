...
├───────────────────────────────────────────────────────────────┤
│ 🔴 活跃 (3)                                                   │
│  ├─ 📌 身世之谜   mystery     [详情][推进][回收][废弃][编辑][删] │
│  │   依赖: 玉佩来历                                            │
│  ├─ 📌 玉佩来历   mystery     [详情][推进][回收][废弃][编辑][删] │
│  └─ 📌 断剑认主   item        [详情][推进][回收][废弃][编辑][删] │
│                                                               │
│ ✅ 已回收 (1)                                                 │
│  └─ 灵根测试                                                  │
│                                                               │
...
- 行操作按钮全部展开（H3：不收 ⋯）：详情（展开 relations 全览：埋点/推进/回收节点列表 + 依赖链 + involves 关联）· 推进 · 回收 · 废弃 · 编辑（data 表单，同 EntityDetail）· 移入回收站（软删，H2：直接执行不弹确认，级联计数在 toast 中展示）。

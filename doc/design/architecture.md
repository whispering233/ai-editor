...
启动流程（Node ≥ 22.12，产物为全仓 ESM）:
  node packages/server/dist/index.js [projectRoot]
    ...
    → 书架模式（2026-08，参考 inkos）：创建书 = 创作根/books/<书名>/ 子目录（三文件），
      ...
      自动备份（决策 27 + 决策 28）：书目录内 .backups/ 存备份 zip（毫秒级时间戳命名，
      手动备份可带自定义名称；旧秒级格式兼容解析；保留 20 份，覆盖恢复前自动快照；
    ...
    → 调试日志（纯配置文件，server/src/debug.ts——`initDebugConfig` 启动读一次）：
      ...
      显式传入 llm 包——显式 true 才开，无 env 回退）/ usage（tokens 统计）/
      http（hono/logger 请求日志，仅该类别挂载）；categories 缺失 = 全部、
      enabled 缺失/false = 全关；无配置文件/非法 JSON/结构不符 = 全关
      （环境变量开关已移除；默认关闭防刷屏）

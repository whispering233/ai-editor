...
- **文件 = 真相源，DB 索引 = 派生镜像（同步方案，2026-08 与用户确认）**：
  ...
  - **外部编辑/新增/删除**（文件管理器直接操作）：scan 端点幂等全量比对——**「已索引跳过」= 索引存在 且 文件 mtime === 索引 `file_mtime` 才跳过**；mtime 不一致 → 以文件为准重新解析 frontmatter + 正文更新索引；文件缺失 → 索引同步软删（进回收站可还原）；软删索引 + 文件回归 references/ → 还原索引。

...
- **自动备份（服务端定时器）**：服务运行期间按频率定时检查，**仅当三文件（outline.json / data.db / project.json）有任一 mtime 晚于「上次备份时刻」才生成新备份**（上次备份时刻 = `.backups/` 中最新备份的时间戳，无状态、服务重启不丢；无变更跳过，不产生垃圾备份）。备份文件格式**复用 E1 zip**（三文件 + wal_checkpoint 完整快照）。

...
| # | 项 | 性质 | 工作量 |
|---|----|------|--------|
| 1 | #2 导出/导入 | 产品承诺违约 + 数据保全最后防线 | ~0.5-1 人日（2-3 卡） |
| 2 | #8 publishConfig.access=public（6 包） | scoped 包发布硬阻断 | 一行 × 6 |
| 3 | #8 npm publish 真实演练 | 流程，一次性 | ~0.5 人日 |
| 4 | #12 未来版本拒绝重建（user_version > SCHEMA_VERSION 分支） | 堵降级数据丢失路径 | ~5 行 + 测试 |
...
1. **导出/导入 zip 库选型**：fflate（纯 JS 零原生依赖，推荐）vs archiver（更重）。
2. **#8 演练方式**：直接 npm publish 真实 registry（无 CI 前提下更贴近真实，可撤回）vs 本地 verdaccio（更安全但额外搭环境）。
3. **迁移机制是否发布窗口内做**：建议做（与首发相隔越久越被动）。

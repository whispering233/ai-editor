...
- **v0.0.19（2026-08-21）**：批次十三发布——**决策 45** 人物列表行信息修订（状态列移除——character.status 无定义且存量恒空致困惑，列表与详情页表单一并移除，存量容错保留、AI 工具 filters.status 语义不变；人物行改**四列布局**：名称+动机摘要 / 角色 / 性格 / 能力独立成列——用户复核修订首版两行式（单 td 与表头错位致角色列空白 + 合并 chips 无法分辨性格能力），toSummary 摘要扩展 motivation 截断 40 / personality·abilities 各前 2）/ **决策 46** 设定树手动排序（修订决策 42「设定无 sort_order」约束：同级组内线性序复用 entities.sort_order 列零迁移、EntitySummary.sortOrder 仅 setting 填充、复合端点 PUT /entity/setting/:id/move 改父+组内重排一次事务（防环沿用决策 30）、排序方式切换器 名称/创建时间/手动、手动模式 ↑↓ 箭头 + 行间插入线拖拽重排、拖到行中段仍 = 调层级）/ 设定树行描述摘要展示；全仓 1657 测试全绿 + typecheck/lint/build 通过；发布前检查清理死代码（DetailFieldConfig.placeholder——status 移除后残留）+ 真实浏览器（Chromium）几何断言验证四列对齐（td 加 flex 致列重叠坑记录 layout.md §4.4）。
- **v0.0.20（2026-08-23）**：批次十四发布——**决策 47** 工具调用展示人类可读化（会话中工具调用行/提案卡不再 JSON dump 原始参数（含裸 id）：新增 `POST /api/v1/names/resolve` 批量名称解析端点（按 id 前缀分流查库：char-/set-/loc-/hook-/ev-/tp-/ref- → 实体、vol-/ch-/sc- → 大纲节点、rel- → null、未知/软删 → null）+ client `summarizeToolCall`/`summarizePreview` 摘要渲染（ToolCallRow 展开态批量解析 id 显示名称、提案卡 preview 摘要化、历史回放同路径、解析失败/未知工具回退原始 JSON 兜底；formatValue 对象数组 → 项数修复 [object Object]））/ **决策 27 修订** 备份频率新增 1 分钟档（BACKUP_FREQUENCIES = [1,5,10,15,30,60]，纯增量）/ **决策 48** 用户级配置格式正式化 schema v1（shared `userConfigFileSchema`：schema_version=1 可选 + model/thinking_level/api_key 宽松读取未知字段保留、旧格式读侧兼容不写回、设置页保存时自然升级；多供应商 v2 用户裁决放弃记录 backlog #17）；全仓 1690 测试全绿 + typecheck/lint/build 通过，14.1-14.4 每卡「实现 + oracle 验证」双代理全 PASS；发布前检查无死代码残留、清理 tasks.md「v0.0.15 待发」等过时文档内容。

...
- [x] **批次十三（决策 45/46，2026-08，已完成待发布 v0.0.19）**：
  - [x] 卡 13.1（bug，决策 45 前半）：人物列表移除状态列（SUMMARY_COLUMNS.character 去 status）+ 详情页 status 输入框 placeholder 引导「如：活跃、退场、已故」`98a86e1`
  - [x] 卡 13.2（UX，决策 45 后半）：人物行两行式布局——服务端 toSummary character 扩展（motivation 截断 40 / personality / abilities 各前 2）+ 前端行渲染（名称+角色徽标 / 动机摘要+性格能力 chips）`5992ff9`
  ...
  - [x] 卡 13.6（bug+UX 修订，2026-08 用户复核）：人物行改四列——角色/性格/能力独立成列（修复首版单 td 与双列表头错位致角色列空白 + 合并 chips 无法分辨性格/能力），动机仍为名称列第二行；状态字段从详情页表单一并移除（列表与详情均不再展示）
    > 坑记录：①首版两行式单 td 与双列表头错位致角色列空白——改四列；②四列首版 td 直接加 `flex` 类被浏览器表格布局塞进同一列槽（性格/能力列重叠，Chromium 实测 left 同为 573px）——改内层 div 承载 flex，红线已补 layout.md §4.4

...
- **完成**：阶段 A 地基 + 切片 1-9、12、13 + 阶段 U（U1-U8）+ 交互修复批次 + 切片 10 画布（S10.1）+ 切片 11 发布（S11.1-S11.3）+ 发布阻断项 E1-E6（导出/导入、未来版本拒绝重建、增量迁移、发布链路 OIDC 全绿）+ 阶段 B 项目提示词编辑（B1，决策 24/25）+ **阶段 C 时间轴（C1-C4，决策 26）** + 画布增强批次（S10.2-S10.5，inkos 参考）+ 交互优化批次（UX1-UX4，用户实测反馈）+ **阶段 B2 自动备份与恢复（B2.1-B2.4，决策 27）** + **B2.5 备份命名增强（毫秒精度 + 自定义名称，决策 28）** + **用户反馈批次 F1-F9（2026-08 实测：Bug 2 项——事件字段清空语义 / 自动备份漏检 data.db-WAL；交互优化 4 项——垂直时间轴 UI / 同标签归组 / 时间标签样式 / 行内完整描述；新需求 3 项——三栏可收起拖宽 / 标签输入建议 / LLM 时间标签排序提案）** + **G1 时间轴区块独立滚动 + G2 时间标签点实体化（决策 26 重大修订：timepoint 实体 + occurs_at 挂载 + 双独立线性序 + SCHEMA_VERSION 3，G2.1-G2.4 全部完成）**——详见「项目演进路线」。
...
- 验证：文档一致性走查
- 回滚：单 commit
- **状态（2026-08）**：✅ 已完成（commit 待填；AGENTS.md 状态行/阅读顺序/三栏布局描述同步 G2 与 F1-F9、CHANGELOG Unreleased 补 G1/G2 条目、tasks.md 项目状态行与测试数更新；G2 四卡全部收官，全仓 1595 用例全绿 + typecheck 0 错误 + lint）。

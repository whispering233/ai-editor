...
- [x] 用户反馈批次六 H3 操作按钮全部展开，禁止收进 `...` 菜单
- [x] 用户反馈批次七 H4 时间轴新建事件按钮横向排布 + 文字型按钮统一加边框
- [x] 用户反馈批次八 H5 时间轴标题行信息与操作右移 + 重命名/新建事件改图标按钮
- [x] 用户反馈批次九 H6 事件行“N 节点”计数靠右
...
**用户反馈批次六（H3，2026-08）**——交互简化：操作按钮一律直接展示，禁止收进 `...`/更多菜单做二级展开。落地：时间轴事件行 `⋯` → 详情/编辑/移入回收站三个图标按钮；伏笔行 `⋯` → 详情/推进/回收/废弃/编辑/移入回收站六个图标按钮；书架项目行 `⋯` → 重命名图标按钮；同步写入 `AGENTS.md` 与 `layout.md` 作为红线。

**用户反馈批次七（H4，2026-08）**——交互与视觉：① 时间轴「+ 在此时间点新建事件」从组尾纵向按钮移到时间标签标题行、“重命名”右侧横向排布；② 全仓文字型操作按钮统一加可见边框（重命名/新建/重试/展开收起/标签筛选/面包屑等），图标按钮不受限；同步写入 `AGENTS.md` 与 `layout.md`。

**用户反馈批次八（H5，2026-08）**——时间轴标题行精简：事件计数、重命名、在此时间点新建事件、移入回收站、折叠全部靠右；重命名与「在此时间点新建事件」由文字按钮改为图标按钮（Pencil / Plus），左侧只保留时间标签文本，减少文字干扰。

**用户反馈批次九（H6，2026-08）**——时间轴事件行「N 节点」计数从名称/tags 旁移到右侧信息与操作区，与详情/编辑/移入回收站按钮一起靠右，减少左侧干扰。
...
| H3 | 操作按钮全部展开 | git log 回溯 | 交互：时间轴/伏笔/书架去除 ⋯ 菜单，直接按钮 |
| H4 | 新建事件按钮横向排布 + 文字按钮加边框 | git log 回溯 | 交互/视觉：标题行内新建 + 全仓文字按钮边框 |
| H5 | 时间轴标题行信息与操作右移 + 改图标按钮 | git log 回溯 | 交互：左侧仅时间标签，右侧计数与操作 |
| H6 | 事件行“N 节点”计数靠右 | git log 回溯 | 交互：右侧信息与操作区 |

...
- [x] **批次十（决策 37-42，2026-08，全部完成）**：
  ...
  - [x] 任务卡 2（决策 37）：大纲交互优化——移除行级「详情」「＋新建」按钮（只留删除）；选中节点按 Enter 新建子级（就地输入行出现在子级末尾，Enter 确认）；双击节点查看详情；点击标题行内编辑（现有单击编辑保留）；拖拽排序保留（HTML5 DnD）
  - [x] 任务卡 3（决策 38）：时间轴交互优化——事件行与组标题行采用大纲交互模式：双击 = 详情、点击标题 = 行内编辑、移除「详情/编辑」按钮（保留删除）；新建/拖拽/折叠/标签筛选等现有能力保留
  - [x] 任务卡 5（决策 41）：AGENTS.md 规则文件——项目目录 AGENTS.md 为项目规则唯一事实源；project.json `prompt` 废弃；打开项目时 prompt 存在且无 AGENTS.md 则自动迁移写入；设置页改为直接编辑 AGENTS.md；web 读取检测外部修改（mtime）；「## 项目设定」注入逻辑保留（数据源改为 AGENTS.md）
  - [x] 任务卡 6（决策 42）：实体设定页树形视图——设定列表改为树形视图（参考大纲页设计）并与设定树 tab 合并；折叠/展开、行内编辑、拖拽调整层级/顺序（belongs_to 防环沿用决策 30）、Enter 新建子级、双击详情；筛选改为搜索 + 标签过滤（树内过滤），移除表格分页
  - [x] 任务卡 7（决策 39）：移除实体二级页列表更新时间——EntityList 表格移除「更新时间」列与排序选项；详情页（EntityDetail 等）创建/更新时间元信息保留

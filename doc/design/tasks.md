...
**用户反馈批次四（H1，2026-08）**——时间轴时间点组标题补齐「移入回收站」垃圾桶图标按钮：点击弹软删确认框（提示其下 N 个事件将变为未挂载），确认后 `DELETE /entity/timepoint/:id`，时间点消失、其下事件移入未挂载区；与事件行/大纲行的删除入口保持一致。
...
| H1 | 时间点删除按钮补齐 | git log 回溯 | 交互：组标题垃圾桶图标 + 软删确认级联提示 |

...
**S9.1 伏笔面板** ✅（09b2c4e，经 oracle 审核放行；复合写裁决：保留三请求方案含 status 同步）
- 范围：伏笔池列表（活跃/已回收分组）、**健康指标徽标（_health 附加字段）——MVP 简化不做（backlog #13：_health 契约未定义，面板不展示健康指标与章节序，见 `doc/ui/pages/hook-panel.md` 第 3 行）**、创建/推进/回收/废弃操作入口（走提案流程）、依赖链展示；**接入三栏布局**——中栏 tab「伏笔」（`#/hooks`，layout.md §1，2026-08 修订）
- 依赖：S8.2、S6.5
- 验证：构造伏笔 fixture 走查全部指标呈现
- 回滚：单 commit

**S9.2 大纲节点伏笔标记** ✅（1973b89，经 oracle 审核放行）

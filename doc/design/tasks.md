...
- [ ] 批次五收尾（K1，2026-08 用户复核）：新建行加回标签输入（逗号分隔 + datalist）+ 详情页 rules 字段label改「标签」

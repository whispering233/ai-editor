...
- **完成**：阶段 A 地基 + 切片 1-9、12、13 + 阶段 U（U1-U8）+ 交互修复批次 + 切片 10 画布（S10.1）+ 切片 11 发布（S11.1-S11.3）+ 发布阻断项 E1-E6（导出/导入、未来版本拒绝重建、增量迁移、发布链路 OIDC 全绿）+ 阶段 B（B1 项目提示词编辑）+ 阶段 C 时间轴（C1-C4，决策 26）+ 画布增强批次（S10.2-S10.5）+ 交互优化批次（UX1-UX4）+ 阶段 B2 自动备份与恢复（B2.1-B2.6，决策 27/28/29）+ **用户反馈批次一 F1-F9（Bug 2 / 交互 4 / 新需求 3，2026-08 实测）** + **用户反馈批次二 G1-G2（G1 区块独立滚动 / G2 时间标签点实体化——决策 26 重大修订：timepoint 实体 + occurs_at 挂载 + 双独立线性序 + SCHEMA_VERSION 3，G2.1-G2.4）** + **用户反馈批次三 G3（操作后滚动位置保持，2026-08 实测）** + **用户反馈批次四 H1（时间点删除按钮补齐，2026-08 实测）** + **用户反馈批次五 H2（软删/还原免二次确认，仅彻底删除保留确认，2026-08 实测）**。
- **待做**：无。可选收尾：npm 坏版本 v0.0.1/v0.0.2 deprecate 标注（需 2FA 凭据，见 AGENTS.md 发布流程段）；backlog.md 事项一律不做。
- **测试**：全仓 1595 个（shared 131 / llm 59 / db 243 / server 324 / client 507 / tools 237 / agent 94）。
...
- [x] 用户反馈批次四 H1 时间轴时间点删除按钮补齐
- [x] 用户反馈批次五 H2 软删/还原免二次确认，仅彻底删除保留确认
...
**用户反馈批次四（H1，2026-08）**——时间轴时间点组标题补齐「移入回收站」垃圾桶图标按钮（H2 起点击直接软删，不再弹确认）：`DELETE /entity/timepoint/:id`，时间点消失、其下事件移入未挂载区；与事件行/大纲行的删除入口保持一致。

**用户反馈批次五（H2，2026-08）**——交互简化：移入回收站（软删）与回收站还原均**不再弹二次确认**，点击直接执行（成功 toast / 失败 toast）；仅回收站「彻底删除」（purge）保留二次确认。涉及大纲、实体详情、伏笔、时间轴列表/详情、回收站页。
...
| H1 | 时间点删除按钮补齐 | git log 回溯 | 交互：组标题垃圾桶图标（H2 起免确认） |
| H2 | 软删/还原免二次确认 | git log 回溯 | 交互：软删与还原直接执行，purge 保留确认 |

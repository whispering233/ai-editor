...
- **完成**：阶段 A 地基 + 切片 1-9、12、13 + 阶段 U（U1-U8）+ 交互修复批次 + 切片 10 画布（S10.1）+ 切片 11 发布（S11.1-S11.3）+ 发布阻断项 E1-E6（导出/导入、未来版本拒绝重建、增量迁移、发布链路 OIDC 全绿）——详见「项目演进路线」。
- **待做**：阶段 B 项目提示词编辑（B1，决策 25——见「阶段 B」卡片区）；可选收尾 = npm 坏版本 v0.0.1/v0.0.2 deprecate 标注（需 2FA 凭据，见 E6 卡）；backlog 事项一律不做。
...
- 验证：手工（编辑保存 → 新对话「## 项目设定」含新内容；清空 → 段消失）
- 回滚：单 commit

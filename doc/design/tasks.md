...
- [x] **批次十四（决策 47/48 + 决策 27 修订，2026-08，发布 v0.0.20）**：14.1-14.5 全部完成（详情见 CHANGELOG.md v0.0.20 段 / release-review.md）；每卡实现 + oracle 验证双代理全 PASS
...
**批次十五（2026-08 已完成并发布 v0.0.22，决策 49：db 查询层 drizzle-orm 集成）**——查询层引入 drizzle-orm（查询构建 + 行类型推断）提升开发体验：不引入 drizzle-kit（迁移管线维持自建 user_version 三态分流，决策 13 E4/E5 不动）；表结构声明收敛 `tables.ts`（sqliteTable 定义 + DDL 常量同文件，schema.test.ts 对齐断言锁）；data 等 JSON 列保持 text 模式 + 行映射层防御解析（drizzle json mode 坏 JSON 抛错弃用）；shared 契约类型不动；签名保持 (db: Db) + queryDb 包装（调用方零改动）；混合风格（builder + sql 模板）逐模块渐进替换，实现层 61 处 prepare 清零；migration 保持 native；全仓 1692 测试全绿。

...
- [ ] **批次十（决策 37-42，2026-08，文档契约已更新，任务卡未开始）**：
  - [ ] 任务卡 1+4（决策 40）：移除全部 6 处行级 AskAiButton（实体/伏笔/参考资料/大纲/时间点/事件）+ 新增右键菜单（context menu）——「注入会话上下文」（复用 chat store focusContext）与「建立关联」（新建 relation_records 关联）；InfoBar「问 AI」统一入口保留

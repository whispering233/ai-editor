...
- **id 约定**：`{前缀}-{nanoid}`（如 `char-9f3k2m`）。前缀表：`char-`/`set-`/`loc-`/`hook-`/`ev-`（实体，`ev-` 为事件）、`sc-`/`ch-`/`vol-`（大纲）、`rel-`（关系，S3.2 补充）、`proj-`（项目）、`prop_`/`sess_`/`call_`（运行时对象）。本文示例中的 `char-9` 等为形状示意，**非自增序号**。
...
导出当前项目完整数据为 zip 备份包（产品承诺「数据主权归用户」的载体，见 `product.md`）。
...
// Req
{
  ...
  target_type: string;
  target_id: string;
  relation_type: string;     // 参见 database/schema.md 预定义关系类型（含 occurs_in 事件锚定）
  metadata?: Record<string, unknown>;
}
...
### 用户级配置文件 `~/.ai-editor/config.json`（正式 schema）
...
| `thinking_level` | enum（可选） | 思考强度 `off/minimal/low/medium/high/xhigh/max`，缺省 `high` |
| `api_key` | string（可选） | DeepSeek API key；空字符串语义 = 清除（仅写入路径） |

**来源优先级**：环境变量 `DEEPSEEK_API_KEY` > config.json `api_key`。文件不存在 / JSON 损坏 / schema 不合法 → 按空配置读取（默认值语义，不抛错）；**不主动改写用户文件**（用户下次在设置页保存时自然落新格式）。

### GET /api/v1/settings/llm

读取 LLM 配置（设置页可配置 DeepSeek key）。

...
- **人物页是工作台**（`#/characters` master-detail，无独立列表页）：右栏 = `components/character/character-detail.tsx`（**四 tab**：人物档案可编辑 / 阅读进度只读 / 人物关系网 / 其他关联 · N——关系两块是**平级 tab**，不在字段 tab 之下；「人物关系网」tab 内列表上方还有 `relation-star-graph`（零依赖手写 SVG，叶子按人合并 + `· N`），阈值/几何口径见 `DESIGN.md`；字段区 = 一个 card 内的**档案式网格**，「基础信息 / 可变数据」分区已删，阅读进度值画纯文本）；泛型 `EntityList`/`EntityDetail` 只服务 setting/location（**已冻结，不再承载新能力**）。新增 antd 组件/样式前先扩 `DESIGN.md` 与 token 守卫。

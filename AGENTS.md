...
- 验证：`pnpm typecheck` / `pnpm lint` / `pnpm -r test`；**单包测试的 filter 必须与 package.json 的 `name` 逐字一致**（`pnpm --filter @whispering233/ai-editor-<pkg> test`；⚠ 桌面包名**无 scope** = `ai-editor-desktop`，加前缀同样空跑）——短名（如 `--filter server`）或写错前缀都会打印 `Scope: 0 of 8 workspace projects` 并**静默什么都不跑**（2026-10 实测踩坑：验收证据因此可能全是空的）；⚠ fresh clone 先 `pnpm -r build` 再 typecheck；UI 改动额外用浏览器核一次像素；**改桌面版主进程（依赖/import）后必须跑打包态启动冒烟**（`build.md`：`typecheck` 绿 ≠ 打包态能起）。
...
- **块文档（正文/参考资料）**：真相 = `document_records.content`（块数组 JSON，章节 id / `ref-*` 为 key），投影 = 服务端派生的 `content_text`（**只能服务端写**，派生失败不得阻断保存）；库写入唯一入口 = db 包的文档 helper（禁止绕过直接写表）；保存正文**不产生 Delta、不推进 `current_position`**；**正文只能挂章**（REST/工具/UI 三面同口径）。**AI 侧只有只读工具**（`get_chapter_text` 单章分页读投影；`get_entity('reference')` 返回投影文本）——任何文档写工具一律不得注册（`@blocknote/xl-*` 类能力禁用）；聚焦章时 `buildFocusText` 注入「标题 + 摘要 + 正文前 2000 字符（标注节选）」，不整章灌入。
- **第三方块编辑器**：`@blocknote/core|react|ariakit` **exact pin**（升级 = 显式 commit + 全量测 + 浏览器像素核对，同 pi 纪律）；**禁止 `@blocknote/xl-*`**（GPL/商业，且 AI 改写能力在包）；改色唯一入口 = `packages/client/src/components/blocknote/blocknote.css`（不得在调用点写 `--bn-*` 覆盖）；**写作面偏好**（字体/字号/行高/纸张/纹理/段首缩进）唯一存储 = `localStorage` `ai-editor:writing`（全局一份，两处书写面共用，不进项目文件/备份/云），下传只经 `--writing-*` / `--paper-*` 变量间接引用（档位常量 = `hooks/use-writing-prefs.ts`，测试断言与 `DESIGN.md` 同值）；写作工具条内置在块编辑器组件（常显、`sticky`），详见 `docs/ui/DESIGN.md` §写作面；`server`/`tools`/`db`/`agent` 不得依赖任何 `@blocknote/*`（服务端不解析块结构）。**参考资料不再有磁盘文件与扫描**（旧 `references/` 目录不读、不打包；外部编辑改走单文件导入导出，md 导入/导出必须先提示有损）。
- **数值单源（2026-10 三连发教训）**：任何上限/缺省/阈值常量**只在定义处出现**——散文与注释一律引用常量名或函数名，**不得复述数字**；模型需要看到的数字（工具 `description`）必须由常量**插值**生成，并有测试断言两者同值。已收口：`get_chapter_text` 的 max_chars、`analyze_hook_health` 的半衰期映射、`search_references` 的摘要截断、`listEntities` 的 limit clamp 等。（同一形状三次复发：改值漏改 description / 漏改 schema 注释 / 注释与实现早已漂移；根治手段是「不复述」而非「记得同步」。）
...
- **大纲页 = 双视图 + 展示口径编号（2026-09）**：`#/outline` 两态「大纲树（默认）/ 章视图」，**页面 state 不持久化**（localStorage 仍只有主题 / 三栏面板 / 写作偏好三个 key）；切换按钮 = 页头「全部折叠」左侧、**文案 = 目标视图**；章视图隐藏「全部折叠」。卷/章行徽标 = `第N卷` / `第N章`（`TypeChip` + `min-w-14` + `tabular-nums`），场景行仍「场」。**编号 = 展示口径**（只计未软删节点 ⇒ 删后重排；纯函数 = `lib/outline-tree.ts` 的 `numberOutline`），**不得**与服务端 `deriveChapterOrder`（含软删、保 Delta/伏笔引用稳定）混用或互为替代（先读 `10-data-model.md` §4）。
  - **行块内三处几何必须同步**：折叠箭头占位（`-ml-2 w-6`）/ 类型徽标（卷章 `min-w-14`、场 `w-7`）/ 就地新建行徽标——漏一处就错位；**就地输入行唯一实现 = `components/outline/inline-input.tsx`**（树视图与章视图共用，别再各写一份）。章视图的交互 = 「改标题 + 进详情 + 写正文（导航链接）」+ 每行字数徽标（`textLength > 0` 才显示），仍**无删除/新建/拖拽**（结构编辑只在树视图；`startCreate` 必须先切回树视图，否则输入行不渲染）；编号不扩散到详情页/选章下拉——见 `backlog.md`。

...
- **块文档（正文/参考资料）**：真相 = `document_records.content`（块数组 JSON，章节 id / `ref-*` 为 key），投影 = 服务端派生的 `content_text`（**只能服务端写**，派生失败不得阻断保存）；库写入唯一入口 = db 包的文档 helper（禁止绕过直接写表）；保存正文**不产生 Delta、不推进 `current_position`**；**正文只能挂章**（REST/工具/UI 三面同口径）。**AI 侧只有只读工具**：任何文档写工具一律不得注册（`xl-ai` 类能力禁用）。
...
- **大纲页 = 双视图 + 展示口径编号（2026-09）**：`#/outline` 两态「大纲树（默认）/ 章视图」，**页面 state 不持久化**（localStorage 仍只有主题与三栏面板两个 key）；切换按钮 = 页头「全部折叠」左侧、**文案 = 目标视图**；章视图隐藏「全部折叠」。卷/章行徽标 = `第N卷` / `第N章`（`TypeChip` + `min-w-14` + `tabular-nums`），场景行仍「场」。**编号 = 展示口径**（只计未软删节点 ⇒ 删后重排；纯函数 = `lib/outline-tree.ts` 的 `numberOutline`），**不得**与服务端 `deriveChapterOrder`（含软删、保 Delta/伏笔引用稳定）混用或互为替代（先读 `10-data-model.md` §4）。
  - **行块内三处几何必须同步**：折叠箭头占位（`-ml-2 w-6`）/ 类型徽标（卷章 `min-w-14`、场 `w-7`）/ 就地新建行徽标——漏一处就错位；**就地输入行唯一实现 = `components/outline/inline-input.tsx`**（树视图与章视图共用，别再各写一份）。章视图有意只保留「改标题 + 进详情」（无删除/新建/拖拽；`startCreate` 必须先切回树视图，否则输入行不渲染）；编号不扩散到详情页/选章下拉——见 `backlog.md`。

...
- **第三方块编辑器**：`@blocknote/core|react|ariakit` **exact pin**（升级 = 显式 commit + 全量测 + 浏览器像素核对，同 pi 纪律）；**禁止 `@blocknote/xl-*`**（GPL/商业，且 AI 改写能力在包）；改色唯一入口 = `packages/client/src/components/blocknote/blocknote.css`（不得在调用点写 `--bn-*` 覆盖）；`server`/`tools`/`db`/`agent` 不得依赖任何 `@blocknote/*`（服务端不解析块结构）。**参考资料不再有磁盘文件与扫描**（旧 `references/` 目录不读、不打包；外部编辑改走单文件导入导出，md 导入/导出必须先提示有损）。
...
- **大纲页 = 双视图 + 展示口径编号（2026-09）**：`#/outline` 两态「大纲树（默认）/ 章视图」，**页面 state 不持久化**（localStorage 仍只有主题与三栏面板两个 key）；切换按钮 = 页头「全部折叠」左侧、**文案 = 目标视图**；章视图隐藏「全部折叠」。卷/章行徽标 = `第N卷` / `第N章`（`TypeChip` + `min-w-14` + `tabular-nums`），场景行仍「场」。**编号 = 展示口径**（只计未软删节点 ⇒ 删后重排；纯函数 = `lib/outline-tree.ts` 的 `numberOutline`），**不得**与服务端 `deriveChapterOrder`（含软删、保 Delta/伏笔引用稳定）混用或互为替代（先读 `10-data-model.md` §4）。

...
![AI Editor 品牌标记](packages/client/public/brand-icon.svg)

面向小说创作者的本地优先 AI 辅助工具。**AI 是创作顾问，不是代笔**——帮助管理创作要素、探索剧情可能性、发现设定矛盾。正文（章级）在应用内用块编辑器书写、随书备份与导入导出，但 **AI 对正文只有只读权限**：能读、能评论、能建议，工具面上不存在改写入口——落笔永远由作者完成。
...
- **`#/` 书架主页**：书籍列表（打开高亮、行内导出/重命名/继续创作/**删除书籍**——已启用云备份时先推一份再删，可选同时删云端备份）+ 新建 / 导入备份 / **从云端恢复** / 打开其他路径（「导出」弹窗二选一：**项目压缩文件**＝三文件 zip，**小说文档**＝卷/章 markdown 目录树 zip，md 有损且不可再导入）
...
- **左栏**：品牌标记 + `书架` 入口 + 书名按钮（项目概览）+ 八项导航（大纲 | 人物 | 设定 | 地点 | 伏笔 | 时间轴 | 关联 | 参考资料）+ 回收站 + 设置 / 主题切换
- **中栏**：信息条（项目名、阅读进度、语言、全局刷新）+ 页面内容区 + 右下悬浮「问 AI」（带当前页面上下文注入右栏）
- **右栏**：AI 聊天常驻（会话随项目目录走、窄屏折叠为抽屉；x `Conversations` 会话列表 + x Bubble / x-markdown 消息流（一条消息内顺序 = 思维链 → 工具调用行 → 正文）+ 提案确认卡；输入框下方 = 配置行（模型选择 / 思考强度）+ **会话状态栏**（上下文占用 · 费用 · 解码速度 · 缓存命中率 · 累计 tokens；只读、窄栏按优先级隐藏，见 `docs/ui/DESIGN.md`））
...
- **剧情推演**：在大纲树上把若干**章**标为**推演节点**（`project.json` 的 `deduction_nodes`，随书备份/云端/恢复；单标记 = 开放式发散，多标记 = 相邻标记之间的剧情线探讨）——树视图/章视图/节点详情页显示派生徽标（`推演节点` / `推演起点` / `推演节点 k` / `推演终点`）；悬浮「问 AI」把标记集合注入上下文，AI 也可自行调 `get_deduction_marks` 取标记与相邻区间骨架；大纲页页头可**一键清除全部标记**（有可见标记时才出现）；**标记是作者意图，AI 只能读、不能改**
...
- **伏笔**：伏笔池面板（活跃 / 已回收 / 已废弃、推进 / 回收 / 废弃复合写确认、依赖链展开）+ 大纲节点伏笔标记（树视图 / 章视图行内，点击跳该伏笔详情）
- **参考资料**：第 7 类实体（`ref-` 前缀），表格列表 + 块编辑器正文 + 分类 / 标签 / URL；Markdown 与块 JSON 导入导出
- **数据备份**：一键导出完整项目 zip（或导出**小说文档**：`{书名}/第N卷 {卷名}/第N章 {章名}.md` 卷/章 markdown 目录树 zip，内联样式有损、**不可再导入**）/ 从备份导入；自动备份（关闭 / 1 / 5 / 10 / 15 / 30 / 60 分钟，有变更才备份，保留最近 20 份）+ 手动备份（可带名称）+ 备份重命名 + 加载备份（覆盖前自动快照）+ **`Ctrl/Cmd + S`** 快捷存档（任意页面 = 保存当前内容并生成本地存档；键位清单见 设置 → 快捷键）；**云端存档（WebDAV）**把备份 zip 同步到用户自己的云盘做异地副本与多设备续写（本地仍是唯一事实源；新机器可用「从云端恢复」拉回整本书）；软删内容走**回收站**（还原 / 彻底清除 + 启动一致性校验）。**备份/项目包导出/云端的包只含三文件**：对话历史 `sessions/` 是纯本地目录（不进包、不跨机器搬运、恢复类操作不碰它；小说文档导出同样不含它，且只含正文）
...
- **AI 设置**：模型目录与凭据全部来自 pi（设置页只列已配置 provider；key 写入 pi agent dir，不入项目文件）；出站请求支持 HTTP 代理（读 pi 全局设置的 `httpProxy`，未设时回落环境变量）

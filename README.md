...
| AI 调用 | `@earendil-works/pi-ai`（统一多提供商 LLM 接口；deepseek + opencode-go 两 provider，模型/思考强度/上下文预算可配置，key 按 provider 三级链独立解析；llm 包单向 adapter 保留对外契约） |
| 测试 | vitest（各包独立 `test` script） |

## UI（v0.0.31 设置页信息架构 + v0.0.30 右栏与选中面 + v0.0.28 视觉契约）
...
- **右栏**：AI 聊天常驻（会话随项目目录走、`<1024px` 折叠为抽屉；会话列表 = x `Conversations` + 逐项「删除会话」（danger 二次确认），消息流 = x Bubble + x-markdown 流式渲染 + 工具调用折叠行 + 提案确认卡；输入框下方一行 = 模型选择 ／ 上下文占用（分母 = 本轮生效预算）+ 思考强度）
...
- `@whispering233/ai-editor-db`：SQLite 建表（drizzle `tables.ts` 双份声明）/ 查询（drizzle-orm 构建器）/ 原子写 / **会话 JSONL 文件存储**（`sessions/*.jsonl`）/ schema 演进（增量迁移 + 未来版本拒绝打开，无迁移路径时删库重建兜底）
...
- **对话历史与会话存储（v0.0.31，2026-09）**：对话历史**从 data.db 迁到项目目录** `sessions/<session_id>.jsonl`（一 session 一文件；行 = header + 消息行，追加写、可读可 diff）——会话随书目录走（备份/导入/改名/移动天然携带），思维链/会话名/每轮模型等扩展为**纯追加**（不做文件级迁移）；`SCHEMA_VERSION 5→6` 迁移 006 自动出库并 `DROP TABLE`（失败整体回滚可重试；id 非法的旧会话改名 `sess_legacy_<hash>` 导出，数据不丢）；新增 `DELETE /chat/sessions/:id`（400/404/409 + 右栏二次确认）；备份/恢复覆盖 `sessions/`。**上下文预算配置化**：`~/.ai-editor/config.json` 新增 `context_budget`（`history_ratio` / `tool_result_max_tokens`），历史预算 = 激活模型 `contextWindow × ratio`（总闸 = `window × 0.5` 派生，替换硬编码 60K）；单条工具结果超限**截断 + 提示**不终止对话；裁剪**不得裁空**。**token 估算改分级密度**（ASCII 4 字符/token、中文 1.7 字符/token）——修掉 `chars/4` 对中文低估 2.4 倍（唯一能击穿预算护栏的路径）；占用条分母改「本轮生效预算」（`done` 帧 `context_budget`）。**设置页信息架构重构**：二级 tab（AI 模型 / 项目规则 / 备份）+ AI 模型三级导航（provider 竖向导航 + key 管理；模型下拉去重，激活唯一入口在聊天栏）
...
- **AI 对话链路（S6-S8）**：llm 引擎换核为 `@earendil-works/pi-ai`（手写 SSE/流式累积删除，对外契约保留）；48 个工具（查询 9 / 分析 5 / 伏笔 5 / 提案 16 / 执行 13 = LLM 可见 35 + 执行 13）；agent 主循环（8 轮 / 120s / token 三重保险）；提案确认流程（卡片确认/拒绝 + 失效处理）；chat SSE 路由（心跳 / 断连检测 / 全链路取消）；**右栏增强**——模型选择下拉（17 模型按 provider optgroup 分组、未配 key 的 provider 整组禁用、跨 provider 选模型自动切 key）+ 思考强度选择（off/minimal/low/medium/high/xhigh/max，对齐 pi）+ 上下文占用进度条（真实 usage ÷ 当前模型 contextWindow）；**消息流渲染**——x Bubble + x-markdown 流式增量渲染（已完成消息不重渲、流式思考指示）、Sender 输入（IME 安全）、工具调用行 Collapse + 状态点、提案卡确认/拒绝；**多 provider**——设置页每提供商一张卡（模型点选即激活 + key 保存/清除/掩码 + 激活高亮），三级 key 解析链（env `DEEPSEEK_API_KEY`/`OPENCODE_API_KEY` > 用户配置 `~/.ai-editor/config.json` api_keys > pi-agent `~/.pi/agent/auth.json` 只读兜底；key 不入项目文件）；模型目录按 provider 隔离、撞名模型靠 provider 消歧，**解析绝不跨 provider**；**问 AI 入口（v0.0.27 迁中栏右下）**——中栏右下悬浮按钮（读当前页面焦点注入右栏 + 聚焦输入框；右栏收起/小屏抽屉自动展开；无项目或未选中条目时给中性提示）+ 行级**右键菜单**（大纲/实体/伏笔/事件/时间点/参考资料行，右键「注入会话上下文」带对象提问 +「建立关联」新建关系；行级 AskAiButton 已移除）——配置 key 后右栏 ChatPanel 可直接对话
...
**发布流程**（详见 `docs/design/build.md`「正式发布链路」）：更新根 `CHANGELOG.md`（Unreleased 搬运为新版本段）→ `pnpm release:version X.Y.Z` 同步 6 包 + client + 根版本 → commit + 手动 annotated tag `vX.Y.Z` → push tag 后 workflow 自动执行（release.yml 建 GitHub Release，publish.yml 发布 6 包 npm + 安装态冒烟验证）。

...
- **剧情推演**：在大纲树上把若干**章**标为**推演节点**（`project.json` 的 `deduction_nodes`，随书备份/云端/恢复；单标记 = 开放式发散，多标记 = 相邻标记之间的剧情线探讨）——树视图/章视图/节点详情页显示派生徽标（`推演节点` / `推演起点` / `推演节点 k` / `推演终点`）；悬浮「问 AI」把标记集合注入上下文，AI 也可自行调 `get_deduction_marks` 取标记与相邻区间骨架；**标记是作者意图，AI 只能读、不能改**
...
- **人物页**：master-detail 工作台——人物档案（可编辑）/ 阅读进度（只读）/ 人物关系网 / 其他关联四个平级 tab；能力面板 = 用户自定义字段树；**角色优先级**（主角 / 主要配角 / 配角 / 龙套，未分级沉底）驱动左栏默认排序
...
- **AI 对话**：37 个 LLM 可见工具（查询 11 / 分析 5 / 伏笔 5 / 提案 16）+ 13 个执行类（用户确认后由服务端执行）；提案确认流程（TTL 10 分钟 + 快照重校验）；SSE = pi 会话事件投影；思维链默认折叠；**AI 只提案不写入**，正文与参考资料只有只读工具

...
面向小说创作者的本地优先 AI 辅助工具。**AI 是创作顾问，不是代笔**——帮助管理创作要素、探索剧情可能性、发现设定矛盾。正文（章级）在应用内用块编辑器书写、随书备份与导入导出，但**AI 对正文只有只读权限**：能读、能评论、能建议，工具面上不存在任何改写正文的入口——落笔永远由作者完成。
...
| 前端 | React 19 + Vite 7 + Zustand 5 + **antd v6**（ConfigProvider zhCN + 浅/深双算法 + cssVar tokens；**主题 = Notion 工作区暖灰 token 覆盖**，视觉契约见 `docs/ui/DESIGN.md`）+ **@ant-design/icons**（唯一图标集）+ **@ant-design/x**（Bubble/Sender 会话组件族）+ **@ant-design/x-markdown**（流式渲染）+ Tailwind 4（仅布局 utility）+ **@blocknote/core·react·ariakit**（块文档编辑器，exact pin）+ Prettier（样式工程化） |
...
- **`#/manuscript/:chapterId` 正文书写面**（入口：大纲页章视图行 / 章详情页「写正文」）：一行**常显写作工具条**（块类型下拉、加粗 / 斜体 / 下划线 / 删除线、颜色、对齐、缩进、链接、撤销 / 重做、保存、写作设置、命令帮助、专注模式）+ **铺满剩余高度**的写作面（点最后一行下方的空白即落文末）；**写作偏好**（字体：无衬线 / 宋体 / 楷体 / 仿宋 / 等宽；字号 14 / 16 / 18 / 20；行高 1.5 / 1.8 / 2.0；纸色：默认 / 米黄 / 暖灰；纸纹理：横线 / 网格；段首缩进两字符）记在本机、所有正文共用；**专注模式**一键收起三栏外壳与页头；**页头常驻**（章标题 + 字数 / 保存态 + 上一章 / 下一章 / 导入 / 导出）；停止输入 ~1.5s 自动保存，另有手动「保存」。参考资料详情页共用同一套编辑器与工具条（无专注模式；编辑态保存按钮在工具条，新建态是底部「取消 / 创建」）。
...
- **中栏**：信息条（项目名→概览、阅读进度、语言、全局刷新、小屏聊天开关）+ 页面内容区（内容区右下角悬浮「问 AI」按钮——点击必有反应：带当前页面上下文注入右栏；右栏收起/小屏抽屉自动打开；**章正文页进入专注模式时该按钮不渲染**——专注态没有右栏，留着只会点空）
...
- **三栏默认宽度**：左 10% / 右 40% / 中栏吸收剩余（两侧各有可读区间：左 160-480、右 240-960）——**1:5:4 在 1600–2400 视口精确成立**，更窄时左栏取下限、更宽时右栏封顶由中栏吸收；可拖拽调宽 + 收起/展开，宽度与收起态持久化 localStorage（另一个展示层 key = 上文的写作偏好 `ai-editor:writing`；大纲页的「大纲树 / 章视图」选择同样记在本机，`ai-editor:outline-view`）
...
- **文档与一致性收口（v0.0.51，2026-09）**：无功能变更——客户端实体列表上限与文档里的常量改为引用单一定义（不再复述数字）、合同文档去掉任务卡编号引用、清掉过时注释与过时计数、修正 `auto-push` 测试的「造变更」手法与未来 mtime、macOS 安装包资产名去空格（潜在更新 404 隐患）、发布流程补 `gh auth` 账号检查。

- **写作面升级 + 四处 UX 收口（v0.0.50，2026-09）**：① **正文写作体验**——常显写作工具条（块类型 / 格式 / 颜色 / 对齐 / 缩进 / 链接 / 撤销重做 / 保存 / 写作设置 / 命令帮助 / 专注模式）、写作偏好（字体·字号·行高·纸色·纸纹理·段首缩进，记本机、所有正文共用）、专注模式（一键收起三栏与页头）、手动「保存」与「已保存 · 12:03」时间戳；② **写作面铺满剩余高度**（短正文不再缩成一小块，点末行下方空白即落文末）+ **章正文页页头常驻**（滚动时标题与「上一章 / 下一章 / 导入 / 导出」不离开视口）；③ **大纲页记住你的视图**（切到「章视图」后跳走 / 刷新仍保持；只记视图，「全部折叠」仍是一次性）；④ **章视图「写正文」改为行尾铅笔图标按钮**；⑤ **参考资料「分类」两处统一为同一枚描边式徽标**（列表列 + 详情页头）；⑥ **关联页的类型徽标挪到名称前面**（源 / 目标两列各自成一列对齐，人物页关系区与实体详情页的「A 关系 B」读法不变）。
- **正文进系统 + 参考资料改写（v0.0.49，2026-09）**：① **章级正文**——大纲页章视图或章详情点「写正文」进入**块编辑器**（标题/列表/引用/代码/表格/图片所见即所得），停止输入片刻自动保存（页头「N 字 已保存」），离开页面立即落盘，失败有可见提示与重试；同一章在两处打开编辑时后保存方会看到冲突提示（重新加载 / 覆盖保存）；正文可**导入导出**（块 JSON 无损、Markdown 有损且先确认）；章视图每行显示该章字数与「写正文」入口。② **参考资料**换成同一套块编辑器（自动保存同款），列表页不再有「扫描」与「未同步」，改为**单文件导入导出**（「导入 md 新建」会取 frontmatter 的 title 作条目名，正文不含 frontmatter），新建只需填名称。③ **AI 可读正文（只读）**——AI 能按需分页读取章正文（长章分段读并告知续读位置）、读取参考资料全文，用于分析节奏、核对与设定的冲突、评价具体段落；对着某一章提问时会自动带上该章开头一段作为开场上下文。**AI 没有任何改写正文的工具**，只能读、评论与建议。④ **存储与备份**——正文与参考资料正文存库（`data.db` 的文档表，不再有项目目录 `references/` 与 Markdown 源码编辑器）；备份包 = 项目三文件 + `sessions/`；正文编辑**不产生变更记录（Delta）**、不自动推进阅读进度。
- **桌面版（Electron 外壳，v0.0.40，2026-09）**：用户下载安装包、装完双击即用（**不需要 Node / npm**）——与 npm CLI 版**共用同一份数据与凭据**（`~/.pi/agent/`、项目目录格式一致，两边能打开同一书库）。要点：① **外壳不是第二产品**——主进程内 in-process 起现有 server（`packages/server` 未改一行），窗口加载 `http://127.0.0.1:<端口>`，端口固定优先（保 `localStorage` 偏好稳定）；② **书库位置**首次启动选（建议值 `<文档>/AI Editor`），存 `<userData>/desktop.json`，设置页「通用」tab 可改（改完自动重启）；③ **原生能力只开一个**——preload（沙箱 CJS）仅暴露目录选择，书架页据此多一个「浏览…」按钮，**浏览器形态零变化**（能力检测无桥即不渲染）；④ **安全基线**——`contextIsolation`/`sandbox` 开、`nodeIntegration` 关，外链一律交系统浏览器、跨源导航被拦、生产包不暴露 devtools；⑤ **日志落盘** `<userData>/logs/ai-editor.log`（桌面用户没有终端，日志是排障唯一线索）；⑥ **打包**——electron-builder（Windows NSIS / macOS dmg / Linux AppImage），`pnpm deploy` 收自包含依赖、better-sqlite3 的 N-API 预编译在 Electron 44 上**免 rebuild**；首版不签名（macOS 首次需右键打开、Windows 有 SmartScreen 提示）。设计与契约：`docs/design/50-desktop.md`。
...
- **云端存档（WebDAV，v0.0.38，2026-09）**：给备份加**异地副本 + 多设备续写**——推送/拉取整包 zip 到用户自己的 WebDAV 云盘（**坚果云原生支持**；其他云盘用 rclone / AList 桥接），**不自建后端**。要点：① **本地仍是唯一事实源**——云端只是「备份的另一块磁盘」，离线可用优先，云端失败不阻塞本地功能；② **凭据只落** `<创作根>/.ai-editor/cloud.json`（0600 明文；不进项目文件 / 备份 zip / 任何 API 响应 / **备份文件名段**——设备名与备份标签等于应用密码一律 400，接口从不回传密码）；③ **地址 = 用户云盘根**（如 `https://dav.jianguoyun.com/dav`），工作根 `<云盘根>/ai-editor/` 由应用自动拼接与创建；④ **推送 = 本地备份 zip 逐字节上传**（`PUT .tmp-<名>` → `MOVE` 成正式名，正式名下永远是完整包；同名同大小幂等跳过；只保留最近 5 份，**带标签的永不清理**）；⑤ **拉取 = 三文件覆盖 + `sessions/` 并集合并**（正文与参考资料在 `data.db` 内，随三文件覆盖）（云端删的删本机、本机删的不复活、本机新增保留），覆盖前自动快照——与本地 restore 的整体覆盖语义**刻意不同**；⑥ **三态状态机**（已同步 / 云端更新 / 本机改动）+ 左栏「同步云端」一键动作与两色角标（冲突 = red、有事可做 = warning、不可达不亮）；⑦ **冲突裁决**：两条路都各留一份备份（保留云端 / 用本机覆盖）；⑧ **自动推送**：2 小时节流（复用备份定时链，不新增定时器），关闭项目与手动备份各触发一次，纯聊天不单独触发；有未备份改动时跳过（不把落后内容静默推上云）。设计与契约：`docs/design/40-cloud-sync.md`、`docs/api/100-api-cloud.md`；真机验收清单见 `docs/design/backlog.md`。
...
- **大纲**：严格三层（卷→章→场景；**2026-09 起章只能挂卷**，`root` 仅接纳卷）增删改移（行级＝新建子级按钮 + 删除按钮，新建按钮在删除左侧；选中按 Enter 新建子级、双击查看详情、点击标题行内编辑、右键菜单注入上下文/建立关联）；页头主操作「+ 新建卷」；**卷/章自动编号（`第N卷` / `第N章`：全书连续章序、只计未软删节点 ⇒ 删后重排，纯展示不参与计算——与服务端章序（含软删、保引用稳定）不同源）+ 双视图**（大纲树 / 章视图；切换按钮在页头「全部折叠」左侧、文案 = 目标视图）——章视图 = 平铺章列表（每行 `第N卷` + `第N章` + 标题 + 摘要 + 伏笔标记 + 阅读进度徽标；单击标题改名、双击进详情，结构编辑仍只在树视图）；节点详情（麦基《故事》结构化字段）
...
- **参考资料**：第 7 种实体类型 reference（`ref-` 前缀）——外部素材 / 灵感笔记（区别于本书正文）；列表为**表格平铺**（标题 / 分类 / 标签 / 来源），交互对齐大纲（点击标题行内编辑、双击详情、右键菜单注入上下文与建立关联）；详情页 = 块编辑器正文 + 名称 / 分类 / 标签 / URL（URL 可选，纯本地笔记留空）；**分类自由文本**（建议项来自项目内已用分类）；**导入导出**支持 Markdown（有损先确认）与块 JSON（无损）；**正文与其它创作数据同库**——随备份 / 导出 / 云同步走同一套机制，软删进回收站、还原原样回来；AI 侧 `search_references`（按标题 / 正文关键词搜索）+ `get_entity` 读全文（均为只读投影文本）；一级导航项 `#/references`（旧 `#/entities/*` 重定向）
...
- **AI 对话链路**：内核 = 嵌入 `@earendil-works/pi-coding-agent`（模型/凭据/会话/重试/上下文压缩/工具派发全部由 pi 承担；本仓提供领域工具、内核提示词与 HTTP/SSE 契约）；36 个 LLM 可见工具（查询 10 / 分析 5 / 伏笔 5 / 提案 16）+ 13 个执行类不经 LLM（用户确认后由服务端执行）；工具参数 schema 用 TypeBox（校验交 pi，非法参数自动喂回自纠）；**无轮次上限与单轮超时**（失控靠用户停止/steering 干预；自动重试与自动压缩归 pi）；提案确认流程（TTL 10 分钟 + 快照重校验 + 卡片确认/拒绝）；SSE = pi 会话事件的轻量投影（事件表见 `docs/api/80-api-chat.md`）；**思维链默认折叠**（流式期间自动展开，历史回看按需拉全文）；**右栏**——模型选择（pi 目录 + 认证状态，未配凭据的 provider 不可选）+ 思考强度（off/minimal/low/medium/high/xhigh/max）+ 上下文占用条 + 工具调用行 + 提案卡；**key 管理**——写入 pi 的 agent dir（`~/.pi/agent/auth.json`；**一家一条且存量凭据优先，环境变量只在该家无条目时兜底**），存量 OAuth 订阅登录不被覆盖；key 不入项目文件；**出站请求**支持 HTTP 代理（`HTTP(S)_PROXY` 环境变量或 pi settings 的 `httpProxy`）与可配空闲超时；**问 AI 入口**——中栏右下悬浮按钮（读当前页面焦点注入右栏）+ 行级右键菜单「注入会话上下文 / 建立关联」
...
- **数据备份（阶段 B2 已就绪）**：一键导出完整项目（zip 打包 project.json + outline.json + data.db（含 WAL 完整快照，含正文与参考资料）+ sessions/）/ 从备份导入（服务端校验 + 原子搬入）；**自动备份**——按频率（关闭/1/5/10/15/30/60 分钟，默认 10 分钟开启，跟随书籍）有变更才备份，每项目保留最近 20 份；**手动备份**——设置页「立即备份」可带自定义名称，列表以简单标签区分手动/自动（B2.5/B2.6）；**备份重命名**——列表行内编辑改名称（时间与类型标签保持）；**加载备份**——设置页历史备份列表（强确认 + 覆盖前自动快照后悔药）或书架导入文件（以 project_id 为 key：匹配 → 覆盖恢复 / 不匹配 → 新书，同名不再 409 可重命名或去重并存）；书架支持重命名书名——「数据主权归用户」（`docs/design/00-master-design.md` 原则 1）
...
- **AI 只提案不写入**：结构化数据（人物/设定/大纲/伏笔…）的写操作一律走「提案确认」；**正文与参考资料正文 AI 只有只读工具**——能读、能评论、能建议，工具面上不存在改写入口

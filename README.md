...
- **`#/` 书架主页**：书籍列表分「小说项目 / 小说拆解」两组（打开高亮、行内导出/重命名/继续创作/**删除书籍**——已启用云备份时先推一份再删，可选同时删云端备份）+ 新建 / 导入备份 / **从云端恢复** / 打开其他路径 / **拆解小说**（选 txt → 切分预览 → 建档拆解；进度页可看「拆解记录」过程时间线、按未拆章「继续拆解」，过程会话在 chat 面板可回看但只读）
...
- **拆解小说**：书架页「拆解小说」入口——选 txt → 预览切分（编码探测 / 章数 / 字数分布 / 警告 / 成本预估）→ 建档（项目 + 卷章大纲 + 正文全量导入）→ **多段并行** LLM 抽取（并发段数可配：创作根 `.ai-editor/config.json` 的 `decompose` 段；批大小带双向预算守卫——超窗 / 超输出自动拆批）→ 归并（别名 / 关系）→ 拆解报告；进度页可见阶段 / 批次 / 单批结果，支持中止续拆与单批重跑（样本目录 `references/`，不入库）

...
- **写作体验三处小改（未发布，2026-10）**：① **大纲页记住你的视图**——切到「章视图」后进章正文、切页、刷新都保持在章视图（只记视图，「全部折叠」仍是一次性）；② **章视图「写正文」改为行尾图标按钮**（铅笔图标，悬停提示「写正文」），行为不变；③ **参考资料「分类」两处统一为同一枚描边式徽标**（列表列 + 详情页头；此前一处是裸文字、一处是另一套灰徽标），与「标签」的彩底 chip 形态区分。

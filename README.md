...
> 版本说明：**当前最新版 v0.0.15**（由 CI OIDC 自动发布，发布全链路自动化已验证）；v0.0.1/v0.0.2 因发布管道缺陷（manifest 残留 `workspace:*` 协议）不可安装，已计划 deprecate 标注；安装时使用 `@whispering233/ai-editor-server@latest` 即可。
...
| 目录 | 内容 |
|------|------|
| `doc/design/` | 产品定位、架构与分包、关键决策 1-36、backlog、任务清单与进度 |
